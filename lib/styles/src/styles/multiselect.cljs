...
(o/defstyled switch :div
  ...
  ([id !data]
   (let [checked (get-in @!data [id :st])]
     [ui/switch-group
      {:as "div"}
      [:div.flex.items-center.gap-4.p-1
...
(rf/reg-event-fx :ok-modal-dialog (fn [{db :db}] {:db (assoc db :show-modal-dialog false)
                                                  :fx [[:dispatch [:app/navigate-to [:r.baksiden]]]]}))
...
(defn multiselect-example []
  (let [data [{}]]
    [:div.p-4
     ...
     [list-exampole]
     [sc/row-end
      [sc/button-cta {:on-click #(rf/dispatch [:show-modal-dialog])} "Book nå!"]]
