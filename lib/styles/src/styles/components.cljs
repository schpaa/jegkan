    ...
    (fn [{:keys [open]}]
      [:<>
       [ui/popover-button
        ...
        #_{:class [:text-white :group :bg-orange-700 :px-3 :py-2 :rounded-md :inline-flex :items-center :text-base :font-medium :hover:text-opacity-100 :focus:outline-none :focus-visible:ring-2 :focus-visible:ring-white :focus-visible:ring-opacity-75
       ...
       [ui/transition
        ...
        [ui/transition-child
         ...
         [ui/popover-panel
          {:static false
           :class  "absolute z-10 w-screen max-w-sm mb-2 transform bottom-12 left-0 sm:px-0 lg:max-w-3xl"}
  ...
  [ui/menu-item
   (fn [{:keys [active]}]
     [:button {:class (into [:group :flex :rounded-md :items-center :w-full :px-2 :py-2 :text-sm]
                            (if active [:bg-red-500 :text-white] [:text-gray-900]))}
      [:> icon
       {:class       [:h-5 :w-5 :mr-2 (if active :text-violet-100 :text-red-600)]
        :aria-hidden true}]
      label])])
         ...
         [:div.fixed.inset-0.overflow-y-autox
          ...
          [:div.px-4.text-center
           ...
           [ui/transition-child
            ...
            [sc/dialog
             [sc/dialog-title "Payment successful"]
