  ...
  [:at-media {:min-width "25rem"}
   ...
   [:& :fixed :top-0 :left-0 :bottom-0  
    [:.closed :h-screen {:pointer-events      :none
                         ...
                         :width               "20rem"
                         :transform           "translateX(-100%)"}]
    [:.open :h-screen {:transition-origin   "top right"
                       :pointer-events      :auto
  ...
  ([{:keys [get-menuopen-fn tabdata get-selected-tab] :as m}]
   (let [open? (get-menuopen-fn)]
     [:div {:on-click #()
      ...
      [sidebar.toolbar/toolbar (conj {:color-map s/color-map} m)]
      ((:content (get tabdata (get-selected-tab))))])))

#_(o/defstyled render :div
    :top-0 :border-l-0 :xs:border-l
    ;:bg-red-500
    [:at-media {:min-width "14rem"
                :max-width "25rem"}
     [:& :fixed
      {:width               "100%"
       ...
       :transition-property "width"
       :background-color    :red}
      [:.open :fixed {:transition-duration        "200ms"
                      :transform                  "translateX(0%)"
    ...
    [:& :left-0 :bottom-0 {:pointer-events :none
                           ...
                           ;:max-width      "20rem"
                           :xoverflow-x    :hidden}
     [:.thing :xbg-white :h-screen :z-300]
     [:.open {:transition-duration        "200ms"
              :transform                  "translateX(0%)"
              :transition-origin          "top left"
              :transition-timing-function "cubic-bezier(0.4, 0, 0.2, 1)"
              :pointer-events             :auto
              :xoverflow-x                :hidden}]
     [:.closed {:transition-duration "200ms"
                :transition-property "transform"
                :transform           "translateX(-100%)"
                :pointer-events      :none}]
     [:.shadows {:box-shadow "var(--shadow-6)"}]]
    #_[:.open :h-screen :bg-white
       {
        ;:width                      "20rem"
        :display                    :block
        :pointer-events             :auto
        ...
        :transition-timing-function "cubic-bezier(0.4, 0, 0.2, 1)"
        :transform                  "translateX(0%)"}]
    #_[:.closed :bg-gray-200
       {:pointer-events      :none
        :transition-origin   "top left"
        :transition-duration "200ms"
        :transition-property "transform"
        :transform           "translateX(-100%)"}]
    ([{:keys [tabdata toggle-menu-open get-menuopen-fn get-selected-tab menu-direction] :as accessors}]
     [:div.shadows.thing
