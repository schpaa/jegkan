...
(o/defstyled slidein-menu :div#maint
  ...
  [:.content  {:height "calc(100% - 4rem)"
               :overflow-y :auto}]
  ([m]
   (let [{:keys [tabdata get-menuopen-fn get-selected-tab]} m
         open? (get-menuopen-fn)]
     [:div {:on-click #()
            :class    (if open? :open :closed)}
      [sc/toolbar sidebar.toolbar/tab-list (conj {:color-map s/color-map} m)]
      [:div.content ((:content (get tabdata (get-selected-tab) {:content (fn [] [:div [l/ppre-x (get-selected-tab) tabdata "NOTHING"]])})))]])))
