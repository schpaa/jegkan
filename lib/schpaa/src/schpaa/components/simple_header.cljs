...
(o/defstyled header :div
  :w-full :sticky :top-0 :z-10 :bg-white
  {:display         :grid
...
(o/defstyled title' :div
  :space-y-1
  :flex :flex-col :flex-grow :justify-center :w-full 
  sc/inter
  [:div.header :text-base :font-medium :text-center]
  [:div.small sc/inter :text-xs :text-gray-400]
  [:.bold :font-medium]
  ([description]
  ...
  ([description username date]
   [:<>
    ...
    [:div.small.bold date " -- "  username]]))

;todo extract/remove (this is also duplicated in jegkan.spa)
(defn username [_ e]
  ...
  (let [i @(rf/subscribe [:app/id])
    ...
    [header
     (if ((:get-menuopen-fn m))
       [:div]
       [sc/toolbutton' :hamburger {:on-click (:toggle-menu-open m)}])

     (if front-page?
       [title' "Forsiden"]
       (let [{:keys [description uid date]} @d]
         [title' description (username nil uid) (some-> date t/instant t/date str)]))
     [sc/toolbutton'
      :house
      {:dim? front-page?
