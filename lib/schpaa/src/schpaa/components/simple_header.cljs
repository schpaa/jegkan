...
(defn render [{:keys [menu-direction] :as m}]
  [header
   [centered-button (:toggle-menu-open m) false :house]
   [:div
    (:description @(rf/subscribe [:some/get-active-item-description]))
    #_(or @(rf/subscribe [:some/get-active-item]) "?")]
   ;(:toggle-menu-open m)
   ;(str ((:get-menuopen-fn m)))
   [centered-button (:toggle-menu-open m) false :cloud]])
