...
#_(o/defstyled overlay-with' :div
    :m-4
    [:div.content :relative :h-full :cursor-default :text-black
     [:>.inside :fixed :inset-0 :bg-pink-500
      {:z-index 60}]]

    ([{:keys [short-timeout modal? on-close modal-dim]} & content]
     [:div.content
      [:div.inside
       [l/ppre on-close]
       content]
      ...
      #_(str content)]
     #_[:div.relative.h-full.cursor-default
        [:div.fixed.inset-0
         {:class    (concat
                      animation-duration
                      [:transition :bg-black]
...
(o/defstyled dialog :div
  ...
  ;[:.content :fixed]
  [:.spaced :space-y-4]
  [:div.rel  :fixed :max-w-sm :mx-auto :z-1500 :inset-x-0 {:pointer-events :none}]
  [:div.content  :relative :top-0
   :p-4 :rounded-b
   {:box-shadow "var(--shadow-6)"
    :color "var(--text1)"
    :background-color "var(--surface3)"}]
  [:.active
   ;:absolute :top-0 :inset-x-0x :max-w-xs :mx-auto :h-auto
   {:transform "translateY(0)"}]
  [:.inactive
   ;:absolute :top-0 :inset-x-0x :max-w-xs :mx-auto :h-auto
   {:transform "translateY(-100%)"}]
  [:.header {:font-size "var(--font-size-3)"
             :font-weight "var(--font-weight-6)"}]
  [:.footer :flex :h-16 :items-center :-mx-4 :-mb-4 :rounded-b :px-4 {:color "var(--text1)"
                                                                      :background-color "var(--surface1)"}]
  ([{:keys [type header form-fn footer dialog-type flags] :as m} {:keys [show-status] :as opts}]
   (let [{:keys [bg bgp bgf bgt]} (get color-map dialog-type)]
     [:div.rel
      [:div.content
       {:class (if show-status :active :inactive)}
       [:div.spaced
      ...
      {:display-name
       ...
       (fn [{:keys [show? config-fn]}]
         (let [;todo Possibly move this into the reagent-render method
           ...
           ;instead of form -> dialog
           [dialog m {:show-status @show-status}]

           #_[:div.fixed.inset-x-1.select-none
