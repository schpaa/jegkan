...
(o/defstyled dlg-ok-cancel :div
  :space-y-8
  ([content {:keys [buttons button-captions ok] :as opts}]
   [:<>
    [:div content]
    (cond-> [sc/row-end]
      (:cancel buttons) (conj [sc/button-danger {:on-click #(nrpk.fsm-helpers/send :e.hide)} (or (and button-captions (button-captions :ok)) "Avbryt")])
      (:ok buttons) (conj [sc/button {:on-click #(do (ok) (nrpk.fsm-helpers/send :e.hide))} (or (and button-captions (button-captions :ok)) "Ok")]))]))

(defn ok-cancel
  "high-level"
  [{:keys [content button-captions ok title] :as m}]
  (modal/form-action
    (conj (dissoc m :content :button-captions :ok)
          {:form-fn (fn [_]
                      (dlg-ok-cancel content
                                     {:ok      ok
                                      :buttons #{:cancel :ok}}))})))
