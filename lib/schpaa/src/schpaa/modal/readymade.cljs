...
(o/defstyled dlg-ok-cancel :div
  ...
  ([content {:keys [buttons button-captions ok] :as opts}]
   [:<>
    [:div content]
    (cond-> [sc/row-end]
      (:cancel buttons) (conj [sc/button
                               {:style {:flex-grow 1}
                                :on-click #(nrpk.fsm-helpers/send :e.hide)}
                               (or (and button-captions (button-captions :ok)) "Avbryt")])
      (:ok buttons) (conj [sc/button-danger
                           {:style {:flex-grow 1}
                            :on-click #(do (ok) (nrpk.fsm-helpers/send :e.hide))}
                           (or (and button-captions (button-captions :ok)) "Ok")]))]))
