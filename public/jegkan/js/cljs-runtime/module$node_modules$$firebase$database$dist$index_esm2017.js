...
".info"===pathGetFront(query._path)?syncTreeRemoveEventRegistration(repo.infoSyncTree_,query,eventRegistration):syncTreeRemoveEventRegistration(repo.serverSyncTree_,query,eventRegistration);eventQueueRaiseEventsAtPath(repo.eventQueue_,query._path,eventRegistration)}function repoLog(repo,...varArgs){let prefix="";repo.persistentConnection_&&(prefix=repo.persistentConnection_.id+":");log(prefix,...varArgs)}function repoCallOnCompleteCallback(repo,callback,status,errorReason){callback&&exceptionGuard(()=>
{if("ok"===status)callback(null);else{const code=(status||"error").toUpperCase();var message=code;errorReason&&(message+=": "+errorReason);message=Error(message);message.code=code;callback(message)}})}function repoStartTransaction(repo,path,transactionUpdate,onComplete,unwatcher,applyLocally){repoLog(repo,"transaction on "+path);transactionUpdate={path,update:transactionUpdate,onComplete,status:null,order:LUIDGenerator(),applyLocally,retryCount:0,unwatcher,abortReason:null,currentWriteId:null,currentInputSnapshot:null,
currentOutputSnapshotRaw:null,currentOutputSnapshotResolved:null};onComplete=syncTreeCalcCompleteEventCache(repo.serverSyncTree_,path,void 0)||ChildrenNode.EMPTY_NODE;transactionUpdate.currentInputSnapshot=onComplete;unwatcher=transactionUpdate.update(onComplete.val());if(void 0===unwatcher){if(transactionUpdate.unwatcher(),transactionUpdate.currentOutputSnapshotRaw=null,transactionUpdate.currentOutputSnapshotResolved=null,transactionUpdate.onComplete)transactionUpdate.onComplete(null,!1,transactionUpdate.currentInputSnapshot)}else{validateFirebaseData("transaction failed: Data returned ",
unwatcher,transactionUpdate.path);transactionUpdate.status=0;applyLocally=treeSubTree(repo.transactionQueueTree_,path);var nodeQueue=applyLocally.node.value||[];nodeQueue.push(transactionUpdate);treeSetValue(applyLocally,nodeQueue);"object"===typeof unwatcher&&null!==unwatcher&&(0,_util.contains)(unwatcher,".priority")?(nodeQueue=(0,_util.safeGet)(unwatcher,".priority"),(0,_util.assert)(isValidPriority(nodeQueue),"Invalid priority returned by transaction. Priority must be a valid string, finite number, server value, or null.")):
nodeQueue=(syncTreeCalcCompleteEventCache(repo.serverSyncTree_,path)||ChildrenNode.EMPTY_NODE).getPriority().val();applyLocally=repoGenerateServerValues(repo);unwatcher=nodeFromJSON(unwatcher,nodeQueue);onComplete=resolveDeferredValueSnapshot(unwatcher,onComplete,applyLocally);transactionUpdate.currentOutputSnapshotRaw=unwatcher;transactionUpdate.currentOutputSnapshotResolved=onComplete;transactionUpdate.currentWriteId=repo.nextWriteId_++;transactionUpdate=syncTreeApplyUserOverwrite(repo.serverSyncTree_,
path,onComplete,transactionUpdate.currentWriteId,transactionUpdate.applyLocally);eventQueueRaiseEventsForChangedPath(repo.eventQueue_,path,transactionUpdate);repoSendReadyTransactions(repo,repo.transactionQueueTree_)}}function repoSendReadyTransactions(repo,node=repo.transactionQueueTree_){node||repoPruneCompletedTransactionsBelowNode(repo,node);if(node.node.value){const queue=repoBuildTransactionQueue(repo,node);(0,_util.assert)(0<queue.length,"Sending zero length transaction queue");queue.every(transaction=>
0===transaction.status)&&repoSendTransactionQueue(repo,treeGetPath(node),queue)}else 0<node.node.childCount&&treeForEachChild(node,childNode=>{repoSendReadyTransactions(repo,childNode)})}function repoSendTransactionQueue(repo,path,queue){var setsToIgnore=queue.map(txn=>txn.currentWriteId),latestState=syncTreeCalcCompleteEventCache(repo.serverSyncTree_,path,setsToIgnore)||ChildrenNode.EMPTY_NODE;setsToIgnore=latestState;latestState=latestState.hash();for(let i=0;i<queue.length;i++){const txn=queue[i];
(0,_util.assert)(0===txn.status,"tryToSendTransactionQueue_: items in queue should all be run.");txn.status=1;txn.retryCount++;const relativePath=newRelativePath(path,txn.path);setsToIgnore=setsToIgnore.updateChild(relativePath,txn.currentOutputSnapshotRaw)}setsToIgnore=setsToIgnore.val(!0);repo.server_.put(path.toString(),setsToIgnore,status=>{repoLog(repo,"transaction put response",{path:path.toString(),status});var events=[];if("ok"===status){status=[];for(let i=0;i<queue.length;i++)queue[i].status=
2,events=events.concat(syncTreeAckUserWrite(repo.serverSyncTree_,queue[i].currentWriteId)),queue[i].onComplete&&status.push(()=>queue[i].onComplete(null,!0,queue[i].currentOutputSnapshotResolved)),queue[i].unwatcher();repoPruneCompletedTransactionsBelowNode(repo,treeSubTree(repo.transactionQueueTree_,path));repoSendReadyTransactions(repo,repo.transactionQueueTree_);eventQueueRaiseEventsForChangedPath(repo.eventQueue_,path,events);for(events=0;events<status.length;events++)exceptionGuard(status[events])}else{if("datastale"===
status)for(status=0;status<queue.length;status++)queue[status].status=3===queue[status].status?4:0;else for(warn("transaction at "+path.toString()+" failed: "+status),events=0;events<queue.length;events++)queue[events].status=4,queue[events].abortReason=status;repoRerunTransactions(repo,path)}},latestState)}function repoRerunTransactions(repo,changedPath){var rootMostTransactionNode=repoGetAncestorTransactionNode(repo,changedPath);changedPath=treeGetPath(rootMostTransactionNode);rootMostTransactionNode=
repoBuildTransactionQueue(repo,rootMostTransactionNode);repoRerunTransactionQueue(repo,rootMostTransactionNode,changedPath);return changedPath}function repoRerunTransactionQueue(repo,queue,path){if(0!==queue.length){var callbacks=[],events=[],setsToIgnore=queue.filter(q=>0===q.status).map(q=>q.currentWriteId);for(let i=0;i<queue.length;i++){const transaction=queue[i];var relativePath=newRelativePath(path,transaction.path);let abortTransaction=!1,abortReason;(0,_util.assert)(null!==relativePath,"rerunTransactionsUnderNode_: relativePath should not be null.");
if(4===transaction.status)abortTransaction=!0,abortReason=transaction.abortReason,events=events.concat(syncTreeAckUserWrite(repo.serverSyncTree_,transaction.currentWriteId,!0));else if(0===transaction.status)if(25<=transaction.retryCount)abortTransaction=!0,abortReason="maxretry",events=events.concat(syncTreeAckUserWrite(repo.serverSyncTree_,transaction.currentWriteId,!0));else{var currentNode=syncTreeCalcCompleteEventCache(repo.serverSyncTree_,transaction.path,setsToIgnore)||ChildrenNode.EMPTY_NODE;
transaction.currentInputSnapshot=currentNode;var newData=queue[i].update(currentNode.val());if(void 0!==newData){validateFirebaseData("transaction failed: Data returned ",newData,transaction.path);relativePath=nodeFromJSON(newData);"object"===typeof newData&&null!=newData&&(0,_util.contains)(newData,".priority")||(relativePath=relativePath.updatePriority(currentNode.getPriority()));newData=transaction.currentWriteId;const serverValues=repoGenerateServerValues(repo);currentNode=resolveDeferredValueSnapshot(relativePath,
currentNode,serverValues);transaction.currentOutputSnapshotRaw=relativePath;transaction.currentOutputSnapshotResolved=currentNode;transaction.currentWriteId=repo.nextWriteId_++;setsToIgnore.splice(setsToIgnore.indexOf(newData),1);events=events.concat(syncTreeApplyUserOverwrite(repo.serverSyncTree_,transaction.path,currentNode,transaction.currentWriteId,transaction.applyLocally));events=events.concat(syncTreeAckUserWrite(repo.serverSyncTree_,newData,!0))}else abortTransaction=!0,abortReason="nodata",
events=events.concat(syncTreeAckUserWrite(repo.serverSyncTree_,transaction.currentWriteId,!0))}eventQueueRaiseEventsForChangedPath(repo.eventQueue_,path,events);events=[];abortTransaction&&(queue[i].status=2,setTimeout(queue[i].unwatcher,0),queue[i].onComplete&&("nodata"===abortReason?callbacks.push(()=>queue[i].onComplete(null,!1,queue[i].currentInputSnapshot)):callbacks.push(()=>queue[i].onComplete(Error(abortReason),!1,null))))}repoPruneCompletedTransactionsBelowNode(repo,repo.transactionQueueTree_);
for(path=0;path<callbacks.length;path++)exceptionGuard(callbacks[path]);repoSendReadyTransactions(repo,repo.transactionQueueTree_)}}function repoGetAncestorTransactionNode(repo,path){let transactionNode=repo.transactionQueueTree_;for(repo=pathGetFront(path);null!==repo&&void 0===transactionNode.node.value;)transactionNode=treeSubTree(transactionNode,repo),path=pathPopFront(path),repo=pathGetFront(path);return transactionNode}function repoBuildTransactionQueue(repo,transactionNode){const transactionQueue=
[];repoAggregateTransactionQueuesForNode(repo,transactionNode,transactionQueue);transactionQueue.sort((a,b)=>a.order-b.order);return transactionQueue}function repoAggregateTransactionQueuesForNode(repo,node,queue){const nodeQueue=node.node.value;if(nodeQueue)for(let i=0;i<nodeQueue.length;i++)queue.push(nodeQueue[i]);treeForEachChild(node,child=>{repoAggregateTransactionQueuesForNode(repo,child,queue)})}function repoPruneCompletedTransactionsBelowNode(repo,node){const queue=node.node.value;if(queue){let to=
0;for(let from=0;from<queue.length;from++)2!==queue[from].status&&(queue[to]=queue[from],to++);queue.length=to;treeSetValue(node,0<queue.length?queue:void 0)}treeForEachChild(node,childNode=>{repoPruneCompletedTransactionsBelowNode(repo,childNode)})}function repoAbortTransactions(repo,path){const affectedPath=treeGetPath(repoGetAncestorTransactionNode(repo,path));path=treeSubTree(repo.transactionQueueTree_,path);treeForEachAncestor(path,node=>{repoAbortTransactionsOnNode(repo,node)});repoAbortTransactionsOnNode(repo,
path);treeForEachDescendant(path,node=>{repoAbortTransactionsOnNode(repo,node)});return affectedPath}function repoAbortTransactionsOnNode(repo,node){const queue=node.node.value;if(queue){const callbacks=[];let events=[],lastSent=-1;for(let i=0;i<queue.length;i++)3!==queue[i].status&&(1===queue[i].status?((0,_util.assert)(lastSent===i-1,"All SENT items should be at beginning of queue."),lastSent=i,queue[i].status=3,queue[i].abortReason="set"):((0,_util.assert)(0===queue[i].status,"Unexpected transaction status in abort"),
queue[i].unwatcher(),events=events.concat(syncTreeAckUserWrite(repo.serverSyncTree_,queue[i].currentWriteId,!0)),queue[i].onComplete&&callbacks.push(queue[i].onComplete.bind(null,Error("set"),!1,null))));-1===lastSent?treeSetValue(node,void 0):queue.length=lastSent+1;eventQueueRaiseEventsForChangedPath(repo.eventQueue_,treeGetPath(node),events);for(repo=0;repo<callbacks.length;repo++)exceptionGuard(callbacks[repo])}}function validateNoPreviousOrderByCall(query,fnName){if(!0===query._orderByCalled)throw Error(fnName+
": You can't combine multiple orderBy calls.");}function validateQueryEndpoints(params){let startNode=null,endNode=null;params.hasStart()&&(startNode=params.getIndexStartValue());params.hasEnd()&&(endNode=params.getIndexEndValue());if(params.getIndex()===KEY_INDEX){if(params.hasStart()){if("[MIN_NAME]"!==params.getIndexStartName())throw Error("Query: When ordering by key, you may only pass one argument to startAt(), endAt(), or equalTo().");if("string"!==typeof startNode)throw Error("Query: When ordering by key, the argument passed to startAt(), startAfter(), endAt(), endBefore(), or equalTo() must be a string.");
}if(params.hasEnd()){if("[MAX_NAME]"!==params.getIndexEndName())throw Error("Query: When ordering by key, you may only pass one argument to startAt(), endAt(), or equalTo().");if("string"!==typeof endNode)throw Error("Query: When ordering by key, the argument passed to startAt(), startAfter(), endAt(), endBefore(), or equalTo() must be a string.");}}else if(params.getIndex()===PRIORITY_INDEX){if(null!=startNode&&!isValidPriority(startNode)||null!=endNode&&!isValidPriority(endNode))throw Error("Query: When ordering by priority, the first argument passed to startAt(), startAfter() endAt(), endBefore(), or equalTo() must be a valid priority value (null, a number, or a string).");
...
exports.goOffline=function(db){db=(0,_util.getModularInstance)(db);db._checkNotDeleted("goOffline");db=db._repo;db.persistentConnection_&&db.persistentConnection_.interrupt("repo_interrupt")};exports.goOnline=function(db){db=(0,_util.getModularInstance)(db);db._checkNotDeleted("goOnline");db=db._repo;db.persistentConnection_&&db.persistentConnection_.resume("repo_interrupt")};exports.increment=function(delta){return{".sv":{increment:delta}}};exports.limitToFirst=function(limit){if("number"!==typeof limit||
Math.floor(limit)!==limit||0>=limit)throw Error("limitToFirst: First argument must be a positive integer.");return new QueryLimitToFirstConstraint(limit)};exports.limitToLast=function(limit){if("number"!==typeof limit||Math.floor(limit)!==limit||0>=limit)throw Error("limitToLast: First argument must be a positive integer.");return new QueryLimitToLastConstraint(limit)};exports.off=function(query,eventType,callback){let container=null;callback=callback?new CallbackContext(callback):null;"value"===
eventType?container=new ValueEventRegistration(callback):eventType&&(container=new ChildEventRegistration(eventType,callback));repoRemoveEventCallbackForQuery(query._repo,query,container)};exports.onChildAdded=function(query,callback,cancelCallbackOrListenOptions,options){return addEventListener(query,"child_added",callback,cancelCallbackOrListenOptions,options)};exports.onChildChanged=function(query,callback,cancelCallbackOrListenOptions,options){return addEventListener(query,"child_changed",callback,
cancelCallbackOrListenOptions,options)};exports.onChildMoved=function(query,callback,cancelCallbackOrListenOptions,options){return addEventListener(query,"child_moved",callback,cancelCallbackOrListenOptions,options)};exports.onChildRemoved=function(query,callback,cancelCallbackOrListenOptions,options){return addEventListener(query,"child_removed",callback,cancelCallbackOrListenOptions,options)};exports.onDisconnect=function(ref){ref=(0,_util.getModularInstance)(ref);return new OnDisconnect(ref._repo,
ref._path)};exports.onValue=onValue;exports.orderByChild=function(path){if("$key"===path)throw Error('orderByChild: "$key" is invalid.  Use orderByKey() instead.');if("$priority"===path)throw Error('orderByChild: "$priority" is invalid.  Use orderByPriority() instead.');if("$value"===path)throw Error('orderByChild: "$value" is invalid.  Use orderByValue() instead.');validatePathString("orderByChild","path",path,!1);return new QueryOrderByChildConstraint(path)};exports.orderByKey=function(){return new QueryOrderByKeyConstraint};
exports.orderByPriority=function(){return new QueryOrderByPriorityConstraint};exports.orderByValue=function(){return new QueryOrderByValueConstraint};exports.push=function(parent,value){parent=(0,_util.getModularInstance)(parent);validateWritablePath("push",parent._path);validateFirebaseDataArg("push",value,parent._path,!0);var now=repoServerTime(parent._repo);const name=nextPushId(now);now=child$jscomp$0(parent,name);const pushRef=child$jscomp$0(parent,name);parent=null!=value?set(pushRef,value).then(()=>
pushRef):Promise.resolve(pushRef);now.then=parent.then.bind(parent);now.catch=parent.then.bind(parent,void 0);return now};exports.query=function(query,...queryConstraints){query=(0,_util.getModularInstance)(query);for(const constraint of queryConstraints)query=constraint._apply(query);return query};exports.ref=ref$jscomp$0;exports.refFromURL=function(db,url){db=(0,_util.getModularInstance)(db);db._checkNotDeleted("refFromURL");url=parseRepoInfo(url,db._repo.repoInfo_.nodeAdmin);validateUrl("refFromURL",
url);const repoInfo=url.repoInfo;db._repo.repoInfo_.isCustomHost()||repoInfo.host===db._repo.repoInfo_.host||fatal("refFromURL: Host name does not match the current database: (found "+repoInfo.host+" but expected "+db._repo.repoInfo_.host+")");return ref$jscomp$0(db,url.path.toString())};exports.remove=function(ref){validateWritablePath("remove",ref._path);return set(ref,null)};exports.runTransaction=function(ref,transactionUpdate,options){var _a;ref=(0,_util.getModularInstance)(ref);validateWritablePath("Reference.transaction",
ref._path);if(".length"===ref.key||".keys"===ref.key)throw"Reference.transaction failed: "+ref.key+" is a read-only object.";options=null!==(_a=null===options||void 0===options?void 0:options.applyLocally)&&void 0!==_a?_a:!0;const deferred=new _util.Deferred;_a=onValue(ref,()=>{});repoStartTransaction(ref._repo,ref._path,transactionUpdate,(error,committed,node)=>{error?deferred.reject(error):(error=new DataSnapshot(node,new ReferenceImpl(ref._repo,ref._path),PRIORITY_INDEX),deferred.resolve(new TransactionResult(committed,
error)))},_a,options);return deferred.promise};exports.serverTimestamp=function(){return SERVER_TIMESTAMP};exports.set=set;exports.setPriority=function(ref,priority){ref=(0,_util.getModularInstance)(ref);validateWritablePath("setPriority",ref._path);validatePriority("setPriority",priority,!1);const deferred=new _util.Deferred;repoSetWithPriority(ref._repo,pathChild(ref._path,".priority"),priority,null,deferred.wrapCallback(()=>{}));return deferred.promise};exports.setWithPriority=function(ref,value,
priority){validateWritablePath("setWithPriority",ref._path);validateFirebaseDataArg("setWithPriority",value,ref._path,!1);validatePriority("setWithPriority",priority,!1);if(".length"===ref.key||".keys"===ref.key)throw"setWithPriority failed: "+ref.key+" is a read-only object.";const deferred=new _util.Deferred;repoSetWithPriority(ref._repo,ref._path,value,priority,deferred.wrapCallback(()=>{}));return deferred.promise};exports.startAfter=function(value,key){validateKey("startAfter","key",key,!0);
return new QueryStartAfterConstraint(value,key)};exports.startAt=function(value=null,key){validateKey("startAt","key",key,!0);return new QueryStartAtConstraint(value,key)};exports.update=function(ref,values){validateFirebaseMergeDataArg("update",values,ref._path,!1);const deferred=new _util.Deferred;repoUpdate(ref._repo,ref._path,values,deferred.wrapCallback(()=>{}));return deferred.promise};exports._validateWritablePath=exports._validatePathString=exports._TEST_ACCESS_hijackHash=exports._TEST_ACCESS_forceRestClient=
exports._ReferenceImpl=exports._QueryParams=exports._QueryImpl=exports.TransactionResult=exports.QueryConstraint=exports.OnDisconnect=exports.Database=exports.DataSnapshot=void 0;var _app=require("module$node_modules$$firebase$app$dist$esm$index_esm2017"),_component=require("module$node_modules$$firebase$component$dist$esm$index_esm2017"),_util=require("module$node_modules$$firebase$util$dist$index_esm2017"),_logger=require("module$node_modules$$firebase$logger$dist$index_cjs");let SDK_VERSION="";
...
class MemoryStorage{constructor(){this.cache_={};this.isInMemoryStorage=!0}set(key,value){null==value?delete this.cache_[key]:this.cache_[key]=value}get(key){return(0,_util.contains)(this.cache_,key)?this.cache_[key]:null}remove(key){delete this.cache_[key]}}const createStoragefor=function(domStorageName){try{if("undefined"!==typeof window&&"undefined"!==typeof window[domStorageName]){const domStorage=window[domStorageName];domStorage.setItem("firebase:sentinel","cache");domStorage.removeItem("firebase:sentinel");
return new DOMStorageWrapper(domStorage)}}catch(e){}return new MemoryStorage},PersistentStorage=createStoragefor("localStorage"),SessionStorage=createStoragefor("sessionStorage"),logClient=new _logger.Logger("@firebase/database"),LUIDGenerator=function(){let id=1;return function(){return id++}}(),sha1=function(str){str=(0,_util.stringToByteArray)(str);const sha1=new _util.Sha1;sha1.update(str);str=sha1.digest();return _util.base64.encodeByteArray(str)},buildLogMessage_=function(...varArgs){let message=
"";for(let i=0;i<varArgs.length;i++){const arg=varArgs[i];message=Array.isArray(arg)||arg&&"object"===typeof arg&&"number"===typeof arg.length?message+buildLogMessage_.apply(null,arg):"object"===typeof arg?message+(0,_util.stringify)(arg):message+arg;message+=" "}return message};let logger$jscomp$0=null,firstLog_=!0;const enableLogging$1=function(logger_,persistent){(0,_util.assert)(!persistent||!0===logger_||!1===logger_,"Can't turn on custom loggers persistently.");!0===logger_?(logClient.logLevel=
_logger.LogLevel.VERBOSE,logger$jscomp$0=logClient.log.bind(logClient),persistent&&SessionStorage.set("logging_enabled",!0)):"function"===typeof logger_?logger$jscomp$0=logger_:(logger$jscomp$0=null,SessionStorage.remove("logging_enabled"))},log=function(...varArgs){!0===firstLog_&&(firstLog_=!1,null===logger$jscomp$0&&!0===SessionStorage.get("logging_enabled")&&enableLogging$1(!0));logger$jscomp$0&&(varArgs=buildLogMessage_.apply(null,varArgs),logger$jscomp$0(varArgs))},logWrapper=function(prefix){return function(...varArgs){log(prefix,
...varArgs)}},error$jscomp$0=function(...varArgs){varArgs="FIREBASE INTERNAL ERROR: "+buildLogMessage_(...varArgs);logClient.error(varArgs)},fatal=function(...varArgs){varArgs=`FIREBASE FATAL ERROR: ${buildLogMessage_(...varArgs)}`;logClient.error(varArgs);throw Error(varArgs);},warn=function(...varArgs){varArgs="FIREBASE WARNING: "+buildLogMessage_(...varArgs);logClient.warn(varArgs)},isInvalidJSONNumber=function(data){return"number"===typeof data&&(data!==data||data===Number.POSITIVE_INFINITY||
data===Number.NEGATIVE_INFINITY)},executeWhenDOMReady=function(fn){if((0,_util.isNodeSdk)()||"complete"===document.readyState)fn();else{let called=!1;const wrappedFn=function(){document.body?called||(called=!0,fn()):setTimeout(wrappedFn,10)};document.addEventListener?(document.addEventListener("DOMContentLoaded",wrappedFn,!1),window.addEventListener("load",wrappedFn,!1)):document.attachEvent&&(document.attachEvent("onreadystatechange",()=>{"complete"===document.readyState&&wrappedFn()}),window.attachEvent("onload",
wrappedFn))}},nameCompare=function(a,b){if(a===b)return 0;if("[MIN_NAME]"===a||"[MAX_NAME]"===b)return-1;if("[MIN_NAME]"===b||"[MAX_NAME]"===a)return 1;const aAsInt=tryParseInt(a),bAsInt=tryParseInt(b);return null!==aAsInt?null!==bAsInt?0===aAsInt-bAsInt?a.length-b.length:aAsInt-bAsInt:-1:null!==bAsInt?1:a<b?-1:1},stringCompare=function(a,b){return a===b?0:a<b?-1:1},requireKey=function(key,obj){if(obj&&key in obj)return obj[key];throw Error("Missing required key ("+key+") in object: "+(0,_util.stringify)(obj));
},ObjectToUniqueKey=function(obj){if("object"!==typeof obj||null===obj)return(0,_util.stringify)(obj);const keys=[];for(var k in obj)keys.push(k);keys.sort();k="{";for(let i=0;i<keys.length;i++)0!==i&&(k+=","),k+=(0,_util.stringify)(keys[i]),k+=":",k+=ObjectToUniqueKey(obj[keys[i]]);return k+"}"},splitStringBySize=function(str,segsize){const len=str.length;if(len<=segsize)return[str];const dataSegs=[];for(let c=0;c<len;c+=segsize)c+segsize>len?dataSegs.push(str.substring(c,len)):dataSegs.push(str.substring(c,
c+segsize));return dataSegs},doubleToIEEE754String=function(v){(0,_util.assert)(!isInvalidJSONNumber(v),"Invalid JSON number");var e;if(0===v){var f=e=0;var s=-Infinity===1/v?1:0}else s=0>v,v=Math.abs(v),v>=Math.pow(2,-1022)?(f=Math.min(Math.floor(Math.log(v)/Math.LN2),1023),e=f+1023,f=Math.round(v*Math.pow(2,52-f)-Math.pow(2,52))):(e=0,f=Math.round(v/Math.pow(2,-1074)));const bits=[];for(v=52;v;--v)bits.push(f%2?1:0),f=Math.floor(f/2);for(v=11;v;--v)bits.push(e%2?1:0),e=Math.floor(e/2);bits.push(s?
1:0);bits.reverse();s=bits.join("");e="";for(v=0;64>v;v+=8)f=parseInt(s.substr(v,8),2).toString(16),1===f.length&&(f="0"+f),e+=f;return e.toLowerCase()},INTEGER_REGEXP_=RegExp("^-?(0*)\\d{1,10}$"),tryParseInt=function(str){return INTEGER_REGEXP_.test(str)&&(str=Number(str),-2147483648<=str&&2147483647>=str)?str:null},exceptionGuard=function(fn){try{fn()}catch(e){setTimeout(()=>{warn("Exception was thrown by user callback.",e.stack||"");throw e;},0)}},setTimeoutNonBlocking=function(fn,time){fn=setTimeout(fn,
time);"object"===typeof fn&&fn.unref&&fn.unref();return fn};class AppCheckTokenProvider{constructor(appName_,appCheckProvider){this.appName_=appName_;this.appCheckProvider=appCheckProvider;(this.appCheck=null===appCheckProvider||void 0===appCheckProvider?void 0:appCheckProvider.getImmediate({optional:!0}))||(null===appCheckProvider||void 0===appCheckProvider?void 0:appCheckProvider.get().then(appCheck=>this.appCheck=appCheck))}getToken(forceRefresh){return this.appCheck?this.appCheck.getToken(forceRefresh):
new Promise((resolve,reject)=>{setTimeout(()=>{this.appCheck?this.getToken(forceRefresh).then(resolve,reject):resolve(null)},0)})}addTokenChangeListener(listener){var _a;null===(_a=this.appCheckProvider)||void 0===_a?void 0:_a.get().then(appCheck=>appCheck.addTokenListener(listener))}notifyForInvalidToken(){warn(`Provided AppCheck credentials for the app named "${this.appName_}" `+"are invalid. This usually indicates your app was not initialized correctly.")}}class FirebaseAuthTokenProvider{constructor(appName_,
firebaseOptions_,authProvider_){this.appName_=appName_;this.firebaseOptions_=firebaseOptions_;this.authProvider_=authProvider_;this.auth_=null;this.auth_=authProvider_.getImmediate({optional:!0});if(!this.auth_)authProvider_.onInit(auth=>this.auth_=auth)}getToken(forceRefresh){return this.auth_?this.auth_.getToken(forceRefresh).catch(error=>error&&"auth/token-not-initialized"===error.code?(log("Got auth/token-not-initialized error.  Treating as null token."),null):Promise.reject(error)):new Promise((resolve,
reject)=>{setTimeout(()=>{this.auth_?this.getToken(forceRefresh).then(resolve,reject):resolve(null)},0)})}addTokenChangeListener(listener){this.auth_?this.auth_.addAuthTokenListener(listener):this.authProvider_.get().then(auth=>auth.addAuthTokenListener(listener))}removeTokenChangeListener(listener){this.authProvider_.get().then(auth=>auth.removeAuthTokenListener(listener))}notifyForInvalidToken(){let errorMessage='Provided authentication credentials for the app named "'+this.appName_+'" are invalid. This usually indicates your app was not initialized correctly. ';
errorMessage="credential"in this.firebaseOptions_?errorMessage+'Make sure the "credential" property provided to initializeApp() is authorized to access the specified "databaseURL" and is from the correct project.':"serviceAccount"in this.firebaseOptions_?errorMessage+'Make sure the "serviceAccount" property provided to initializeApp() is authorized to access the specified "databaseURL" and is from the correct project.':errorMessage+'Make sure the "apiKey" and "databaseURL" properties provided to initializeApp() match the values provided for your app at https://console.firebase.google.com/.';
warn(errorMessage)}}class EmulatorTokenProvider{constructor(accessToken){this.accessToken=accessToken}getToken(forceRefresh){return Promise.resolve({accessToken:this.accessToken})}addTokenChangeListener(listener){listener(this.accessToken)}removeTokenChangeListener(listener){}notifyForInvalidToken(){}}EmulatorTokenProvider.OWNER="owner";const FORGE_DOMAIN_RE=/(console\.firebase|firebase-console-\w+\.corp|firebase\.corp)\.google\.com/;class RepoInfo{constructor(host,secure,namespace,webSocketOnly,
nodeAdmin=!1,persistenceKey="",includeNamespaceInQueryParams=!1){this.secure=secure;this.namespace=namespace;this.webSocketOnly=webSocketOnly;this.nodeAdmin=nodeAdmin;this.persistenceKey=persistenceKey;this.includeNamespaceInQueryParams=includeNamespaceInQueryParams;this._host=host.toLowerCase();this._domain=this._host.substr(this._host.indexOf(".")+1);this.internalHost=PersistentStorage.get("host:"+host)||this._host}isCacheableHost(){return"s-"===this.internalHost.substr(0,2)}isCustomHost(){return"firebaseio.com"!==
this._domain&&"firebaseio-demo.com"!==this._domain}get host(){return this._host}set host(newHost){newHost!==this.internalHost&&(this.internalHost=newHost,this.isCacheableHost()&&PersistentStorage.set("host:"+this._host,this.internalHost))}toString(){let str=this.toURLString();this.persistenceKey&&(str+="\x3c"+this.persistenceKey+"\x3e");return str}toURLString(){return`${this.secure?"https://":"http://"}${this.host}/${this.includeNamespaceInQueryParams?`?ns=${this.namespace}`:""}`}}class StatsCollection{constructor(){this.counters_=
{}}incrementCounter(name,amount=1){(0,_util.contains)(this.counters_,name)||(this.counters_[name]=0);this.counters_[name]+=amount}get(){return(0,_util.deepCopy)(this.counters_)}}const collections={},reporters={};class PacketReceiver{constructor(onMessage_){this.onMessage_=onMessage_;this.pendingResponses=[];this.currentResponseNum=0;this.closeAfterResponse=-1;this.onClose=null}closeAfter(responseNum,callback){this.closeAfterResponse=responseNum;this.onClose=callback;this.closeAfterResponse<this.currentResponseNum&&
(this.onClose(),this.onClose=null)}handleResponse(requestNum,data){for(this.pendingResponses[requestNum]=data;this.pendingResponses[this.currentResponseNum];){const toProcess=this.pendingResponses[this.currentResponseNum];delete this.pendingResponses[this.currentResponseNum];for(let i=0;i<toProcess.length;++i)toProcess[i]&&exceptionGuard(()=>{this.onMessage_(toProcess[i])});if(this.currentResponseNum===this.closeAfterResponse){this.onClose&&(this.onClose(),this.onClose=null);break}this.currentResponseNum++}}}
class BrowserPollConnection{constructor(connId,repoInfo,applicationId,appCheckToken,authToken,transportSessionId,lastSessionId){this.connId=connId;this.repoInfo=repoInfo;this.applicationId=applicationId;this.appCheckToken=appCheckToken;this.authToken=authToken;this.transportSessionId=transportSessionId;this.lastSessionId=lastSessionId;this.bytesReceived=this.bytesSent=0;this.everConnected_=!1;this.log_=logWrapper(connId);this.stats_=statsManagerGetCollection(repoInfo);this.urlFn=params=>{this.appCheckToken&&
(params.ac=this.appCheckToken);return repoInfoConnectionURL(repoInfo,"long_polling",params)}}open(onMessage,onDisconnect){this.curSegmentNum=0;this.onDisconnect_=onDisconnect;this.myPacketOrderer=new PacketReceiver(onMessage);this.isClosed_=!1;this.connectTimeoutTimer_=setTimeout(()=>{this.log_("Timed out trying to connect.");this.onClosed_();this.connectTimeoutTimer_=null},3E4);executeWhenDOMReady(()=>{if(!this.isClosed_){this.scriptTagHolder=new FirebaseIFrameScriptHolder((...args)=>{const [command,
arg1,arg2]=args;this.incrementIncomingBytes_(args);if(this.scriptTagHolder)if(this.connectTimeoutTimer_&&(clearTimeout(this.connectTimeoutTimer_),this.connectTimeoutTimer_=null),this.everConnected_=!0,"start"===command)this.id=arg1,this.password=arg2;else if("close"===command)if(arg1)this.scriptTagHolder.sendNewPolls=!1,this.myPacketOrderer.closeAfter(arg1,()=>{this.onClosed_()});else this.onClosed_();else throw Error("Unrecognized command received: "+command);},(...args)=>{const [pN,data]=args;this.incrementIncomingBytes_(args);
this.myPacketOrderer.handleResponse(pN,data)},()=>{this.onClosed_()},this.urlFn);var urlParams={start:"t"};urlParams.ser=Math.floor(1E8*Math.random());this.scriptTagHolder.uniqueCallbackIdentifier&&(urlParams.cb=this.scriptTagHolder.uniqueCallbackIdentifier);urlParams.v="5";this.transportSessionId&&(urlParams.s=this.transportSessionId);this.lastSessionId&&(urlParams.ls=this.lastSessionId);this.applicationId&&(urlParams.p=this.applicationId);this.appCheckToken&&(urlParams.ac=this.appCheckToken);"undefined"!==
...
!1:BrowserPollConnection.forceAllow_?!0:!BrowserPollConnection.forceDisallow_&&"undefined"!==typeof document&&null!=document.createElement&&!("object"===typeof window&&window.chrome&&window.chrome.extension&&!/^chrome/.test(window.location.href))&&!("object"===typeof Windows&&"object"===typeof Windows.UI)}markConnectionHealthy(){}shutdown_(){this.isClosed_=!0;this.scriptTagHolder&&(this.scriptTagHolder.close(),this.scriptTagHolder=null);this.myDisconnFrame&&(document.body.removeChild(this.myDisconnFrame),
this.myDisconnFrame=null);this.connectTimeoutTimer_&&(clearTimeout(this.connectTimeoutTimer_),this.connectTimeoutTimer_=null)}onClosed_(){this.isClosed_||(this.log_("Longpoll is closing itself"),this.shutdown_(),this.onDisconnect_&&(this.onDisconnect_(this.everConnected_),this.onDisconnect_=null))}close(){this.isClosed_||(this.log_("Longpoll is being closed."),this.shutdown_())}send(data){data=(0,_util.stringify)(data);this.bytesSent+=data.length;this.stats_.incrementCounter("bytes_sent",data.length);
data=(0,_util.base64Encode)(data);data=splitStringBySize(data,1840);for(let i=0;i<data.length;i++)this.scriptTagHolder.enqueueSegment(this.curSegmentNum,data.length,data[i]),this.curSegmentNum++}addDisconnectPingFrame(id,pw){if(!(0,_util.isNodeSdk)()){this.myDisconnFrame=document.createElement("iframe");var urlParams={dframe:"t"};urlParams.id=id;urlParams.pw=pw;this.myDisconnFrame.src=this.urlFn(urlParams);this.myDisconnFrame.style.display="none";document.body.appendChild(this.myDisconnFrame)}}incrementIncomingBytes_(args){args=
(0,_util.stringify)(args).length;this.bytesReceived+=args;this.stats_.incrementCounter("bytes_received",args)}}class FirebaseIFrameScriptHolder{constructor(commandCB,onMessageCB,onDisconnect,urlFn){this.onDisconnect=onDisconnect;this.urlFn=urlFn;this.outstandingRequests=new Set;this.pendingSegs=[];this.currentSerial=Math.floor(1E8*Math.random());this.sendNewPolls=!0;if((0,_util.isNodeSdk)())this.commandCB=commandCB,this.onMessageCB=onMessageCB;else{this.uniqueCallbackIdentifier=LUIDGenerator();window["pLPCommand"+
...
e.stack&&log(e.stack),log(e)}}}static createIFrame_(){const iframe=document.createElement("iframe");iframe.style.display="none";if(document.body){document.body.appendChild(iframe);try{iframe.contentWindow.document||log("No IE domain setting required")}catch(e){iframe.src="javascript:void((function(){document.open();document.domain\x3d'"+document.domain+"';document.close();})())"}}else throw"Document body has not initialized. Wait to initialize Firebase until after the document is ready.";iframe.contentDocument?
iframe.doc=iframe.contentDocument:iframe.contentWindow?iframe.doc=iframe.contentWindow.document:iframe.document&&(iframe.doc=iframe.document);return iframe}close(){this.alive=!1;this.myIFrame&&(this.myIFrame.doc.body.innerHTML="",setTimeout(()=>{null!==this.myIFrame&&(document.body.removeChild(this.myIFrame),this.myIFrame=null)},0));const onDisconnect=this.onDisconnect;onDisconnect&&(this.onDisconnect=null,onDisconnect())}startLongPoll(id,pw){this.myID=id;this.myPW=pw;for(this.alive=!0;this.newRequest_(););
}newRequest_(){if(this.alive&&this.sendNewPolls&&this.outstandingRequests.size<(0<this.pendingSegs.length?2:1)){this.currentSerial++;var urlParams={};urlParams.id=this.myID;urlParams.pw=this.myPW;urlParams.ser=this.currentSerial;urlParams=this.urlFn(urlParams);let curDataString="",i=0;for(;0<this.pendingSegs.length;)if(1870>=this.pendingSegs[0].d.length+30+curDataString.length){const theSeg=this.pendingSegs.shift();curDataString=curDataString+"\x26seg"+i+"\x3d"+theSeg.seg+"\x26ts"+i+"\x3d"+theSeg.ts+
"\x26d"+i+"\x3d"+theSeg.d;i++}else break;urlParams+=curDataString;this.addLongPollTag_(urlParams,this.currentSerial);return!0}return!1}enqueueSegment(segnum,totalsegs,data){this.pendingSegs.push({seg:segnum,ts:totalsegs,d:data});this.alive&&this.newRequest_()}addLongPollTag_(url,serial){this.outstandingRequests.add(serial);const doNewRequest=()=>{this.outstandingRequests.delete(serial);this.newRequest_()},keepaliveTimeout=setTimeout(doNewRequest,25E3);this.addTag(url,()=>{clearTimeout(keepaliveTimeout);
doNewRequest()})}addTag(url,loadCB){(0,_util.isNodeSdk)()?this.doNodeLongPoll(url,loadCB):setTimeout(()=>{try{if(this.sendNewPolls){var newScript=this.myIFrame.doc.createElement("script");newScript.type="text/javascript";newScript.async=!0;newScript.src=url;newScript.onload=newScript.onreadystatechange=function(){const rstate=newScript.readyState;rstate&&"loaded"!==rstate&&"complete"!==rstate||(newScript.onload=newScript.onreadystatechange=null,newScript.parentNode&&newScript.parentNode.removeChild(newScript),
loadCB())};newScript.onerror=()=>{log("Long-poll script failed to load: "+url);this.sendNewPolls=!1;this.close()};this.myIFrame.doc.body.appendChild(newScript)}}catch(e){}},1)}}let WebSocketImpl=null;"undefined"!==typeof MozWebSocket?WebSocketImpl=MozWebSocket:"undefined"!==typeof WebSocket&&(WebSocketImpl=WebSocket);class WebSocketConnection{constructor(connId,repoInfo,applicationId,appCheckToken,authToken,transportSessionId,lastSessionId){this.connId=connId;this.applicationId=applicationId;this.appCheckToken=
appCheckToken;this.authToken=authToken;this.frames=this.keepaliveTimer=null;this.bytesReceived=this.bytesSent=this.totalFrames=0;this.log_=logWrapper(this.connId);this.stats_=statsManagerGetCollection(repoInfo);this.connURL=WebSocketConnection.connectionURL_(repoInfo,transportSessionId,lastSessionId,appCheckToken);this.nodeAdmin=repoInfo.nodeAdmin}static connectionURL_(repoInfo,transportSessionId,lastSessionId,appCheckToken){const urlParams={v:"5"};!(0,_util.isNodeSdk)()&&"undefined"!==typeof location&&
location.hostname&&FORGE_DOMAIN_RE.test(location.hostname)&&(urlParams.r="f");transportSessionId&&(urlParams.s=transportSessionId);lastSessionId&&(urlParams.ls=lastSessionId);appCheckToken&&(urlParams.ac=appCheckToken);return repoInfoConnectionURL(repoInfo,"websocket",urlParams)}open(onMessage,onDisconnect){this.onDisconnect=onDisconnect;this.onMessage=onMessage;this.log_("Websocket connecting to "+this.connURL);this.everConnected_=!1;PersistentStorage.set("previous_websocket_failure",!0);try{if((0,_util.isNodeSdk)()){const options=
{headers:{"User-Agent":`Firebase/${"5"}/${SDK_VERSION}/${process.platform}/${this.nodeAdmin?"AdminNode":"Node"}`,"X-Firebase-GMPID":this.applicationId||""}};this.authToken&&(options.headers.Authorization=`Bearer ${this.authToken}`);this.appCheckToken&&(options.headers["X-Firebase-AppCheck"]=this.appCheckToken);const env=process.env,proxy=0===this.connURL.indexOf("wss://")?env.HTTPS_PROXY||env.https_proxy:env.HTTP_PROXY||env.http_proxy;proxy&&(options.proxy={origin:proxy});this.mySock=new WebSocketImpl(this.connURL,
[],options)}else this.mySock=new WebSocketImpl(this.connURL,[],{headers:{"X-Firebase-GMPID":this.applicationId||"","X-Firebase-AppCheck":this.appCheckToken||""}})}catch(e){this.log_("Error instantiating WebSocket.");(onMessage=e.message||e.data)&&this.log_(onMessage);this.onClosed_();return}this.mySock.onopen=()=>{this.log_("Websocket connected.");this.everConnected_=!0};this.mySock.onclose=()=>{this.log_("Websocket connection was disconnected.");this.mySock=null;this.onClosed_()};this.mySock.onmessage=
m=>{this.handleIncomingFrame(m)};this.mySock.onerror=e=>{this.log_("WebSocket error.  Closing connection.");(e=e.message||e.data)&&this.log_(e);this.onClosed_()}}start(){}static forceDisallow(){WebSocketConnection.forceDisallow_=!0}static isAvailable(){let isOldAndroid=!1;if("undefined"!==typeof navigator&&navigator.userAgent){const oldAndroidMatch=navigator.userAgent.match(/Android ([0-9]{0,}\.[0-9]{0,})/);oldAndroidMatch&&1<oldAndroidMatch.length&&4.4>parseFloat(oldAndroidMatch[1])&&(isOldAndroid=
!0)}return!isOldAndroid&&null!==WebSocketImpl&&!WebSocketConnection.forceDisallow_}static previouslyFailed(){return PersistentStorage.isInMemoryStorage||!0===PersistentStorage.get("previous_websocket_failure")}markConnectionHealthy(){PersistentStorage.remove("previous_websocket_failure")}appendFrame_(data){this.frames.push(data);this.frames.length===this.totalFrames&&(data=this.frames.join(""),this.frames=null,data=(0,_util.jsonEval)(data),this.onMessage(data))}handleNewFrameCount_(frameCount){this.totalFrames=
frameCount;this.frames=[]}extractFrameCount_(data){(0,_util.assert)(null===this.frames,"We already have a frame buffer");if(6>=data.length){const frameCount=Number(data);if(!isNaN(frameCount))return this.handleNewFrameCount_(frameCount),null}this.handleNewFrameCount_(1);return data}handleIncomingFrame(mess){null!==this.mySock&&(mess=mess.data,this.bytesReceived+=mess.length,this.stats_.incrementCounter("bytes_received",mess.length),this.resetKeepAlive(),null!==this.frames?this.appendFrame_(mess):
(mess=this.extractFrameCount_(mess),null!==mess&&this.appendFrame_(mess)))}send(data){this.resetKeepAlive();data=(0,_util.stringify)(data);this.bytesSent+=data.length;this.stats_.incrementCounter("bytes_sent",data.length);data=splitStringBySize(data,16384);1<data.length&&this.sendString_(String(data.length));for(let i=0;i<data.length;i++)this.sendString_(data[i])}shutdown_(){this.isClosed_=!0;this.keepaliveTimer&&(clearInterval(this.keepaliveTimer),this.keepaliveTimer=null);this.mySock&&(this.mySock.close(),
this.mySock=null)}onClosed_(){this.isClosed_||(this.log_("WebSocket is closing itself"),this.shutdown_(),this.onDisconnect&&(this.onDisconnect(this.everConnected_),this.onDisconnect=null))}close(){this.isClosed_||(this.log_("WebSocket is being closed"),this.shutdown_())}resetKeepAlive(){clearInterval(this.keepaliveTimer);this.keepaliveTimer=setInterval(()=>{this.mySock&&this.sendString_("0");this.resetKeepAlive()},45E3)}sendString_(str){try{this.mySock.send(str)}catch(e){this.log_("Exception thrown from WebSocket.send():",
e.message||e.data,"Closing connection."),setTimeout(this.onClosed_.bind(this),0)}}}WebSocketConnection.responsesRequiredToBeHealthy=2;WebSocketConnection.healthyTimeout=3E4;class TransportManager{constructor(repoInfo){this.initTransports_(repoInfo)}static get ALL_TRANSPORTS(){return[BrowserPollConnection,WebSocketConnection]}initTransports_(repoInfo){const isWebSocketsAvailable=WebSocketConnection&&WebSocketConnection.isAvailable();let isSkipPollConnection=isWebSocketsAvailable&&!WebSocketConnection.previouslyFailed();
repoInfo.webSocketOnly&&(isWebSocketsAvailable||warn("wss:// URL used, but browser isn't known to support websockets.  Trying anyway."),isSkipPollConnection=!0);if(isSkipPollConnection)this.transports_=[WebSocketConnection];else{repoInfo=this.transports_=[];for(const transport of TransportManager.ALL_TRANSPORTS)transport&&transport.isAvailable()&&repoInfo.push(transport)}}initialTransport(){if(0<this.transports_.length)return this.transports_[0];throw Error("No transports available");}upgradeTransport(){return 1<
this.transports_.length?this.transports_[1]:null}}class Connection{constructor(id,repoInfo_,applicationId_,appCheckToken_,authToken_,onMessage_,onReady_,onDisconnect_,onKill_,lastSessionId){this.id=id;this.repoInfo_=repoInfo_;this.applicationId_=applicationId_;this.appCheckToken_=appCheckToken_;this.authToken_=authToken_;this.onMessage_=onMessage_;this.onReady_=onReady_;this.onDisconnect_=onDisconnect_;this.onKill_=onKill_;this.lastSessionId=lastSessionId;this.connectionCount=0;this.pendingDataMessages=
[];this.state_=0;this.log_=logWrapper("c:"+this.id+":");this.transportManager_=new TransportManager(repoInfo_);this.log_("Connection created");this.start_()}start_(){var conn=this.transportManager_.initialTransport();this.conn_=new conn(this.nextTransportId_(),this.repoInfo_,this.applicationId_,this.appCheckToken_,this.authToken_,null,this.lastSessionId);this.primaryResponsesRequired_=conn.responsesRequiredToBeHealthy||0;const onMessageReceived=this.connReceiver_(this.conn_),onConnectionLost=this.disconnReceiver_(this.conn_);
this.rx_=this.tx_=this.conn_;this.secondaryConn_=null;this.isHealthy_=!1;setTimeout(()=>{this.conn_&&this.conn_.open(onMessageReceived,onConnectionLost)},0);conn=conn.healthyTimeout||0;0<conn&&(this.healthyTimeout_=setTimeoutNonBlocking(()=>{this.healthyTimeout_=null;this.isHealthy_||(this.conn_&&102400<this.conn_.bytesReceived?(this.log_("Connection exceeded healthy timeout but has received "+this.conn_.bytesReceived+" bytes.  Marking connection healthy."),this.isHealthy_=!0,this.conn_.markConnectionHealthy()):
this.conn_&&10240<this.conn_.bytesSent?this.log_("Connection exceeded healthy timeout but has sent "+this.conn_.bytesSent+" bytes.  Leaving connection alive."):(this.log_("Closing unhealthy connection after timeout."),this.close()))},Math.floor(conn)))}nextTransportId_(){return"c:"+this.id+":"+this.connectionCount++}disconnReceiver_(conn){return everConnected=>{if(conn===this.conn_)this.onConnectionLost_(everConnected);else conn===this.secondaryConn_?(this.log_("Secondary connection lost."),this.onSecondaryConnectionLost_()):
this.log_("closing an old connection")}}connReceiver_(conn){return message=>{if(2!==this.state_)if(conn===this.rx_)this.onPrimaryMessageReceived_(message);else if(conn===this.secondaryConn_)this.onSecondaryMessageReceived_(message);else this.log_("message on old connection")}}sendRequest(dataMsg){this.sendData_({t:"d",d:dataMsg})}tryCleanupConnection(){this.tx_===this.secondaryConn_&&this.rx_===this.secondaryConn_&&(this.log_("cleaning up and promoting a connection: "+this.secondaryConn_.connId),
this.conn_=this.secondaryConn_,this.secondaryConn_=null)}onSecondaryControl_(controlData){"t"in controlData&&(controlData=controlData.t,"a"===controlData?this.upgradeIfSecondaryHealthy_():"r"===controlData?(this.log_("Got a reset on secondary, closing it"),this.secondaryConn_.close(),this.tx_!==this.secondaryConn_&&this.rx_!==this.secondaryConn_||this.close()):"o"===controlData&&(this.log_("got pong on secondary."),this.secondaryResponsesRequired_--,this.upgradeIfSecondaryHealthy_()))}onSecondaryMessageReceived_(parsedData){const layer=
requireKey("t",parsedData);parsedData=requireKey("d",parsedData);if("c"===layer)this.onSecondaryControl_(parsedData);else if("d"===layer)this.pendingDataMessages.push(parsedData);else throw Error("Unknown protocol layer: "+layer);}upgradeIfSecondaryHealthy_(){0>=this.secondaryResponsesRequired_?(this.log_("Secondary connection is healthy."),this.isHealthy_=!0,this.secondaryConn_.markConnectionHealthy(),this.proceedWithUpgrade_()):(this.log_("sending ping on secondary."),this.secondaryConn_.send({t:"c",
d:{t:"p",d:{}}}))}proceedWithUpgrade_(){this.secondaryConn_.start();this.log_("sending client ack on secondary");this.secondaryConn_.send({t:"c",d:{t:"a",d:{}}});this.log_("Ending transmission on primary");this.conn_.send({t:"c",d:{t:"n",d:{}}});this.tx_=this.secondaryConn_;this.tryCleanupConnection()}onPrimaryMessageReceived_(parsedData){const layer=requireKey("t",parsedData);parsedData=requireKey("d",parsedData);if("c"===layer)this.onControl_(parsedData);else if("d"===layer)this.onDataMessage_(parsedData)}onDataMessage_(message){this.onPrimaryResponse_();
this.onMessage_(message)}onPrimaryResponse_(){this.isHealthy_||(this.primaryResponsesRequired_--,0>=this.primaryResponsesRequired_&&(this.log_("Primary connection is healthy."),this.isHealthy_=!0,this.conn_.markConnectionHealthy()))}onControl_(controlData){var cmd=requireKey("t",controlData);if("d"in controlData)if(controlData=controlData.d,"h"===cmd)this.onHandshake_(controlData);else if("n"===cmd){this.log_("recvd end transmission on primary");this.rx_=this.secondaryConn_;for(cmd=0;cmd<this.pendingDataMessages.length;++cmd)this.onDataMessage_(this.pendingDataMessages[cmd]);
this.pendingDataMessages=[];this.tryCleanupConnection()}else if("s"===cmd)this.onConnectionShutdown_(controlData);else if("r"===cmd)this.onReset_(controlData);else"e"===cmd?error$jscomp$0("Server Error: "+controlData):"o"===cmd?(this.log_("got pong on primary."),this.onPrimaryResponse_(),this.sendPingOnPrimaryIfNecessary_()):error$jscomp$0("Unknown control packet command: "+cmd)}onHandshake_(handshake){const timestamp=handshake.ts,version=handshake.v,host=handshake.h;this.sessionId=handshake.s;this.repoInfo_.host=
host;0===this.state_&&(this.conn_.start(),this.onConnectionEstablished_(this.conn_,timestamp),"5"!==version&&warn("Protocol version mismatch detected"),this.tryStartUpgrade_())}tryStartUpgrade_(){const conn=this.transportManager_.upgradeTransport();conn&&this.startUpgrade_(conn)}startUpgrade_(conn){this.secondaryConn_=new conn(this.nextTransportId_(),this.repoInfo_,this.applicationId_,this.appCheckToken_,this.authToken_,this.sessionId);this.secondaryResponsesRequired_=conn.responsesRequiredToBeHealthy||
0;conn=this.connReceiver_(this.secondaryConn_);const onDisconnect=this.disconnReceiver_(this.secondaryConn_);this.secondaryConn_.open(conn,onDisconnect);setTimeoutNonBlocking(()=>{this.secondaryConn_&&(this.log_("Timed out trying to upgrade."),this.secondaryConn_.close())},6E4)}onReset_(host){this.log_("Reset packet received.  New host: "+host);this.repoInfo_.host=host;1===this.state_?this.close():(this.closeConnections_(),this.start_())}onConnectionEstablished_(conn,timestamp){this.log_("Realtime connection established.");
this.conn_=conn;this.state_=1;this.onReady_&&(this.onReady_(timestamp,this.sessionId),this.onReady_=null);0===this.primaryResponsesRequired_?(this.log_("Primary connection is healthy."),this.isHealthy_=!0):setTimeoutNonBlocking(()=>{this.sendPingOnPrimaryIfNecessary_()},5E3)}sendPingOnPrimaryIfNecessary_(){this.isHealthy_||1!==this.state_||(this.log_("sending ping on primary."),this.sendData_({t:"c",d:{t:"p",d:{}}}))}onSecondaryConnectionLost_(){const conn=this.secondaryConn_;this.secondaryConn_=
null;this.tx_!==conn&&this.rx_!==conn||this.close()}onConnectionLost_(everConnected){this.conn_=null;everConnected||0!==this.state_?1===this.state_&&this.log_("Realtime connection lost."):(this.log_("Realtime connection failed."),this.repoInfo_.isCacheableHost()&&(PersistentStorage.remove("host:"+this.repoInfo_.host),this.repoInfo_.internalHost=this.repoInfo_.host));this.close()}onConnectionShutdown_(reason){this.log_("Connection shutdown command received. Shutting down...");this.onKill_&&(this.onKill_(reason),
this.onKill_=null);this.onDisconnect_=null;this.close()}sendData_(data){if(1!==this.state_)throw"Connection is not connected";this.tx_.send(data)}close(){2!==this.state_&&(this.log_("Closing realtime connection."),this.state_=2,this.closeConnections_(),this.onDisconnect_&&(this.onDisconnect_(),this.onDisconnect_=null))}closeConnections_(){this.log_("Shutting down all connections");this.conn_&&(this.conn_.close(),this.conn_=null);this.secondaryConn_&&(this.secondaryConn_.close(),this.secondaryConn_=
null);this.healthyTimeout_&&(clearTimeout(this.healthyTimeout_),this.healthyTimeout_=null)}}class ServerActions{put(pathString,data,onComplete,hash){}merge(pathString,data,onComplete,hash){}refreshAuthToken(token){}refreshAppCheckToken(token){}onDisconnectPut(pathString,data,onComplete){}onDisconnectMerge(pathString,data,onComplete){}onDisconnectCancel(pathString,onComplete){}reportStats(stats){}}class EventEmitter{constructor(allowedEvents_){this.allowedEvents_=allowedEvents_;this.listeners_={};
(0,_util.assert)(Array.isArray(allowedEvents_)&&0<allowedEvents_.length,"Requires a non-empty array")}trigger(eventType,...varArgs){if(Array.isArray(this.listeners_[eventType])){eventType=[...this.listeners_[eventType]];for(let i=0;i<eventType.length;i++)eventType[i].callback.apply(eventType[i].context,varArgs)}}on(eventType,callback,context){this.validateEventType_(eventType);this.listeners_[eventType]=this.listeners_[eventType]||[];this.listeners_[eventType].push({callback,context});(eventType=
this.getInitialEvent(eventType))&&callback.apply(context,eventType)}off(eventType,callback,context){this.validateEventType_(eventType);eventType=this.listeners_[eventType]||[];for(let i=0;i<eventType.length;i++)if(eventType[i].callback===callback&&(!context||context===eventType[i].context)){eventType.splice(i,1);break}}validateEventType_(eventType){(0,_util.assert)(this.allowedEvents_.find(et=>et===eventType),"Unknown event: "+eventType)}}class OnlineMonitor extends EventEmitter{constructor(){super(["online"]);
this.online_=!0;"undefined"===typeof window||"undefined"===typeof window.addEventListener||(0,_util.isMobileCordova)()||(window.addEventListener("online",()=>{this.online_||(this.online_=!0,this.trigger("online",!0))},!1),window.addEventListener("offline",()=>{this.online_&&(this.online_=!1,this.trigger("online",!1))},!1))}static getInstance(){return new OnlineMonitor}getInitialEvent(eventType){(0,_util.assert)("online"===eventType,"Unknown event type: "+eventType);return[this.online_]}currentlyOnline(){return this.online_}}
class Path{constructor(pathOrString,pieceNum){if(void 0===pieceNum){this.pieces_=pathOrString.split("/");pathOrString=0;for(pieceNum=0;pieceNum<this.pieces_.length;pieceNum++)0<this.pieces_[pieceNum].length&&(this.pieces_[pathOrString]=this.pieces_[pieceNum],pathOrString++);this.pieces_.length=pathOrString;this.pieceNum_=0}else this.pieces_=pathOrString,this.pieceNum_=pieceNum}toString(){let pathString="";for(let i=this.pieceNum_;i<this.pieces_.length;i++)""!==this.pieces_[i]&&(pathString+="/"+this.pieces_[i]);
return pathString||"/"}}class ValidationPath{constructor(path,errorPrefix_){this.errorPrefix_=errorPrefix_;this.parts_=pathSlice(path,0);this.byteLength_=Math.max(1,this.parts_.length);for(path=0;path<this.parts_.length;path++)this.byteLength_+=(0,_util.stringLength)(this.parts_[path]);validationPathCheckValid(this)}}class VisibilityMonitor extends EventEmitter{constructor(){super(["visible"]);let hidden,visibilityChange;"undefined"!==typeof document&&"undefined"!==typeof document.addEventListener&&
("undefined"!==typeof document.hidden?(visibilityChange="visibilitychange",hidden="hidden"):"undefined"!==typeof document.mozHidden?(visibilityChange="mozvisibilitychange",hidden="mozHidden"):"undefined"!==typeof document.msHidden?(visibilityChange="msvisibilitychange",hidden="msHidden"):"undefined"!==typeof document.webkitHidden&&(visibilityChange="webkitvisibilitychange",hidden="webkitHidden"));this.visible_=!0;visibilityChange&&document.addEventListener(visibilityChange,()=>{const visible=!document[hidden];
visible!==this.visible_&&(this.visible_=visible,this.trigger("visible",visible))},!1)}static getInstance(){return new VisibilityMonitor}getInitialEvent(eventType){(0,_util.assert)("visible"===eventType,"Unknown event type: "+eventType);return[this.visible_]}}class PersistentConnection extends ServerActions{constructor(repoInfo_,applicationId_,onDataUpdate_,onConnectStatus_,onServerInfoUpdate_,authTokenProvider_,appCheckTokenProvider_,authOverride_){super();this.repoInfo_=repoInfo_;this.applicationId_=
applicationId_;this.onDataUpdate_=onDataUpdate_;this.onConnectStatus_=onConnectStatus_;this.onServerInfoUpdate_=onServerInfoUpdate_;this.authTokenProvider_=authTokenProvider_;this.appCheckTokenProvider_=appCheckTokenProvider_;this.authOverride_=authOverride_;this.id=PersistentConnection.nextPersistentConnectionId_++;this.log_=logWrapper("p:"+this.id+":");this.interruptReasons_={};this.listens=new Map;this.outstandingPuts_=[];this.outstandingGets_=[];this.outstandingGetCount_=this.outstandingPutCount_=
0;this.onDisconnectRequestQueue_=[];this.connected_=!1;this.reconnectDelay_=1E3;this.maxReconnectDelay_=3E5;this.establishConnectionTimer_=this.lastSessionId=this.securityDebugCallback_=null;this.visible_=!1;this.requestCBHash_={};this.requestNumber_=0;this.appCheckToken_=this.authToken_=this.realtime_=null;this.forceTokenRefresh_=!1;this.invalidAppCheckTokenCount_=this.invalidAuthTokenCount_=0;this.firstConnection_=!0;this.lastConnectionEstablishedTime_=this.lastConnectionAttemptTime_=null;if(authOverride_&&
!(0,_util.isNodeSdk)())throw Error("Auth override specified in options, but not supported on non Node.js platforms");VisibilityMonitor.getInstance().on("visible",this.onVisible_,this);if(-1===repoInfo_.host.indexOf("fblocal"))OnlineMonitor.getInstance().on("online",this.onOnline_,this)}sendRequest(action,body,onResponse){const curReqNum=++this.requestNumber_;action={r:curReqNum,a:action,b:body};this.log_((0,_util.stringify)(action));(0,_util.assert)(this.connected_,"sendRequest call when we're not connected not allowed.");
this.realtime_.sendRequest(action);onResponse&&(this.requestCBHash_[curReqNum]=onResponse)}get(query){this.initConnection_();const deferred=new _util.Deferred,request={p:query._path.toString(),q:query._queryObject},outstandingGet={action:"g",request,onComplete:message=>{const payload=message.d;"ok"===message.s?(this.onDataUpdate_(request.p,payload,!1,null),deferred.resolve(payload)):deferred.reject(payload)}};this.outstandingGets_.push(outstandingGet);this.outstandingGetCount_++;const index=this.outstandingGets_.length-
1;this.connected_||setTimeout(()=>{const get=this.outstandingGets_[index];void 0!==get&&outstandingGet===get&&(delete this.outstandingGets_[index],this.outstandingGetCount_--,0===this.outstandingGetCount_&&(this.outstandingGets_=[]),this.log_("get "+index+" timed out on connection"),deferred.reject(Error("Client is offline.")))},3E3);this.connected_&&this.sendGet_(index);return deferred.promise}listen(query,currentHashFn,tag,onComplete){this.initConnection_();const queryId=query._queryIdentifier,
pathString=query._path.toString();this.log_("Listen called for "+pathString+" "+queryId);this.listens.has(pathString)||this.listens.set(pathString,new Map);(0,_util.assert)(query._queryParams.isDefault()||!query._queryParams.loadsAllData(),"listen() called for non-default but complete query");(0,_util.assert)(!this.listens.get(pathString).has(queryId),"listen() called twice for same path/queryId.");query={onComplete,hashFn:currentHashFn,query,tag};this.listens.get(pathString).set(queryId,query);this.connected_&&
this.sendListen_(query)}sendGet_(index){const get=this.outstandingGets_[index];this.sendRequest("g",get.request,message=>{delete this.outstandingGets_[index];this.outstandingGetCount_--;0===this.outstandingGetCount_&&(this.outstandingGets_=[]);if(get.onComplete)get.onComplete(message)})}sendListen_(listenSpec){const query=listenSpec.query,pathString=query._path.toString(),queryId=query._queryIdentifier;this.log_("Listen on "+pathString+" for "+queryId);const req={p:pathString};listenSpec.tag&&(req.q=
query._queryObject,req.t=listenSpec.tag);req.h=listenSpec.hashFn();this.sendRequest("q",req,message=>{const payload=message.d,status=message.s;PersistentConnection.warnOnListenWarnings_(payload,query);if((this.listens.get(pathString)&&this.listens.get(pathString).get(queryId))===listenSpec&&(this.log_("listen response",message),"ok"!==status&&this.removeListen_(pathString,queryId),listenSpec.onComplete))listenSpec.onComplete(status,payload)})}static warnOnListenWarnings_(payload,query){payload&&"object"===
typeof payload&&(0,_util.contains)(payload,"w")&&(payload=(0,_util.safeGet)(payload,"w"),Array.isArray(payload)&&~payload.indexOf("no_index")&&(payload='".indexOn": "'+query._queryParams.getIndex().toString()+'"',warn("Using an unspecified index. Your data will be downloaded and "+`filtered on the client. Consider adding ${payload} at `+`${query._path.toString()} to your security rules for better performance.`)))}refreshAuthToken(token){this.authToken_=token;this.log_("Auth token refreshed");this.authToken_?
this.tryAuth():this.connected_&&this.sendRequest("unauth",{},()=>{});this.reduceReconnectDelayIfAdminCredential_(token)}reduceReconnectDelayIfAdminCredential_(credential){if(credential&&40===credential.length||(0,_util.isAdmin)(credential))this.log_("Admin auth credential detected.  Reducing max reconnect time."),this.maxReconnectDelay_=3E4}refreshAppCheckToken(token){this.appCheckToken_=token;this.log_("App check token refreshed");this.appCheckToken_?this.tryAppCheck():this.connected_&&this.sendRequest("unappeck",
{},()=>{})}tryAuth(){if(this.connected_&&this.authToken_){const token=this.authToken_,authMethod=(0,_util.isValidFormat)(token)?"auth":"gauth",requestData={cred:token};null===this.authOverride_?requestData.noauth=!0:"object"===typeof this.authOverride_&&(requestData.authvar=this.authOverride_);this.sendRequest(authMethod,requestData,res=>{const status=res.s;res=res.d||"error";if(this.authToken_===token)if("ok"===status)this.invalidAuthTokenCount_=0;else this.onAuthRevoked_(status,res)})}}tryAppCheck(){this.connected_&&
this.appCheckToken_&&this.sendRequest("appcheck",{token:this.appCheckToken_},res=>{const status=res.s;res=res.d||"error";if("ok"===status)this.invalidAppCheckTokenCount_=0;else this.onAppCheckRevoked_(status,res)})}unlisten(query,tag){const pathString=query._path.toString(),queryId=query._queryIdentifier;this.log_("Unlisten called for "+pathString+" "+queryId);(0,_util.assert)(query._queryParams.isDefault()||!query._queryParams.loadsAllData(),"unlisten() called for non-default but complete query");
this.removeListen_(pathString,queryId)&&this.connected_&&this.sendUnlisten_(pathString,queryId,query._queryObject,tag)}sendUnlisten_(pathString,queryId,queryObj,tag){this.log_("Unlisten on "+pathString+" for "+queryId);pathString={p:pathString};tag&&(pathString.q=queryObj,pathString.t=tag);this.sendRequest("n",pathString)}onDisconnectPut(pathString,data,onComplete){this.initConnection_();this.connected_?this.sendOnDisconnect_("o",pathString,data,onComplete):this.onDisconnectRequestQueue_.push({pathString,
action:"o",data,onComplete})}onDisconnectMerge(pathString,data,onComplete){this.initConnection_();this.connected_?this.sendOnDisconnect_("om",pathString,data,onComplete):this.onDisconnectRequestQueue_.push({pathString,action:"om",data,onComplete})}onDisconnectCancel(pathString,onComplete){this.initConnection_();this.connected_?this.sendOnDisconnect_("oc",pathString,null,onComplete):this.onDisconnectRequestQueue_.push({pathString,action:"oc",data:null,onComplete})}sendOnDisconnect_(action,pathString,
data,onComplete){pathString={p:pathString,d:data};this.log_("onDisconnect "+action,pathString);this.sendRequest(action,pathString,response=>{onComplete&&setTimeout(()=>{onComplete(response.s,response.d)},0)})}put(pathString,data,onComplete,hash){this.putInternal("p",pathString,data,onComplete,hash)}merge(pathString,data,onComplete,hash){this.putInternal("m",pathString,data,onComplete,hash)}putInternal(action,pathString,data,onComplete,hash){this.initConnection_();data={p:pathString,d:data};void 0!==
hash&&(data.h=hash);this.outstandingPuts_.push({action,request:data,onComplete});this.outstandingPutCount_++;action=this.outstandingPuts_.length-1;this.connected_?this.sendPut_(action):this.log_("Buffering put: "+pathString)}sendPut_(index){const action=this.outstandingPuts_[index].action,request=this.outstandingPuts_[index].request,onComplete=this.outstandingPuts_[index].onComplete;this.outstandingPuts_[index].queued=this.connected_;this.sendRequest(action,request,message=>{this.log_(action+" response",
message);delete this.outstandingPuts_[index];this.outstandingPutCount_--;0===this.outstandingPutCount_&&(this.outstandingPuts_=[]);onComplete&&onComplete(message.s,message.d)})}reportStats(stats){this.connected_&&(stats={c:stats},this.log_("reportStats",stats),this.sendRequest("s",stats,result=>{"ok"!==result.s&&this.log_("reportStats","Error sending stats: "+result.d)}))}onDataMessage_(message){if("r"in message){this.log_("from server: "+(0,_util.stringify)(message));const reqNum=message.r,onResponse=
this.requestCBHash_[reqNum];onResponse&&(delete this.requestCBHash_[reqNum],onResponse(message.b))}else{if("error"in message)throw"A server-side error has occurred: "+message.error;if("a"in message)this.onDataPush_(message.a,message.b)}}onDataPush_(action,body){this.log_("handleServerMessage",action,body);if("d"===action)this.onDataUpdate_(body.p,body.d,!1,body.t);else if("m"===action)this.onDataUpdate_(body.p,body.d,!0,body.t);else if("c"===action)this.onListenRevoked_(body.p,body.q);else if("ac"===
action)this.onAuthRevoked_(body.s,body.d);else if("apc"===action)this.onAppCheckRevoked_(body.s,body.d);else if("sd"===action)this.onSecurityDebugPacket_(body);else error$jscomp$0("Unrecognized action received from server: "+(0,_util.stringify)(action)+"\nAre you using the latest client?")}onReady_(timestamp,sessionId){this.log_("connection ready");this.connected_=!0;this.lastConnectionEstablishedTime_=(new Date).getTime();this.handleTimestamp_(timestamp);this.lastSessionId=sessionId;this.firstConnection_&&
this.sendConnectStats_();this.restoreState_();this.firstConnection_=!1;this.onConnectStatus_(!0)}scheduleConnect_(timeout){(0,_util.assert)(!this.realtime_,"Scheduling a connect when we're already connected/ing?");this.establishConnectionTimer_&&clearTimeout(this.establishConnectionTimer_);this.establishConnectionTimer_=setTimeout(()=>{this.establishConnectionTimer_=null;this.establishConnection_()},Math.floor(timeout))}initConnection_(){!this.realtime_&&this.firstConnection_&&this.scheduleConnect_(0)}onVisible_(visible){visible&&
!this.visible_&&this.reconnectDelay_===this.maxReconnectDelay_&&(this.log_("Window became visible.  Reducing delay."),this.reconnectDelay_=1E3,this.realtime_||this.scheduleConnect_(0));this.visible_=visible}onOnline_(online){online?(this.log_("Browser went online."),this.reconnectDelay_=1E3,this.realtime_||this.scheduleConnect_(0)):(this.log_("Browser went offline.  Killing connection."),this.realtime_&&this.realtime_.close())}onRealtimeDisconnect_(){this.log_("data client disconnected");this.connected_=
!1;this.realtime_=null;this.cancelSentTransactions_();this.requestCBHash_={};if(this.shouldReconnect_()){this.visible_?this.lastConnectionEstablishedTime_&&(3E4<(new Date).getTime()-this.lastConnectionEstablishedTime_&&(this.reconnectDelay_=1E3),this.lastConnectionEstablishedTime_=null):(this.log_("Window isn't visible.  Delaying reconnect."),this.reconnectDelay_=this.maxReconnectDelay_,this.lastConnectionAttemptTime_=(new Date).getTime());var timeSinceLastConnectAttempt=(new Date).getTime()-this.lastConnectionAttemptTime_;
timeSinceLastConnectAttempt=Math.max(0,this.reconnectDelay_-timeSinceLastConnectAttempt);timeSinceLastConnectAttempt*=Math.random();this.log_("Trying to reconnect in "+timeSinceLastConnectAttempt+"ms");this.scheduleConnect_(timeSinceLastConnectAttempt);this.reconnectDelay_=Math.min(this.maxReconnectDelay_,1.3*this.reconnectDelay_)}this.onConnectStatus_(!1)}establishConnection_(){const $jscomp$async$this=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if($jscomp$async$this.shouldReconnect_()){$jscomp$async$this.log_("Making a connection attempt");
$jscomp$async$this.lastConnectionAttemptTime_=(new Date).getTime();$jscomp$async$this.lastConnectionEstablishedTime_=null;const onDataMessage=$jscomp$async$this.onDataMessage_.bind($jscomp$async$this),onReady=$jscomp$async$this.onReady_.bind($jscomp$async$this),onDisconnect=$jscomp$async$this.onRealtimeDisconnect_.bind($jscomp$async$this),connId=$jscomp$async$this.id+":"+PersistentConnection.nextConnectionId_++,lastSessionId=$jscomp$async$this.lastSessionId;let canceled=!1,connection=null;const closeFn=
function(){connection?connection.close():(canceled=!0,onDisconnect())};$jscomp$async$this.realtime_={close:closeFn,sendRequest:function(msg){(0,_util.assert)(connection,"sendRequest call when we're not connected not allowed.");connection.sendRequest(msg)}};const forceRefresh=$jscomp$async$this.forceTokenRefresh_;$jscomp$async$this.forceTokenRefresh_=!1;try{const [authToken,appCheckToken]=yield Promise.all([$jscomp$async$this.authTokenProvider_.getToken(forceRefresh),$jscomp$async$this.appCheckTokenProvider_.getToken(forceRefresh)]);
canceled?log("getToken() completed but was canceled"):(log("getToken() completed. Creating connection."),$jscomp$async$this.authToken_=authToken&&authToken.accessToken,$jscomp$async$this.appCheckToken_=appCheckToken&&appCheckToken.token,connection=new Connection(connId,$jscomp$async$this.repoInfo_,$jscomp$async$this.applicationId_,$jscomp$async$this.appCheckToken_,$jscomp$async$this.authToken_,onDataMessage,onReady,onDisconnect,reason=>{warn(reason+" ("+$jscomp$async$this.repoInfo_.toString()+")");
$jscomp$async$this.interrupt("server_kill")},lastSessionId))}catch(error){$jscomp$async$this.log_("Failed to get token: "+error),canceled||($jscomp$async$this.repoInfo_.nodeAdmin&&warn(error),closeFn())}}})}interrupt(reason){log("Interrupting connection for reason: "+reason);this.interruptReasons_[reason]=!0;if(this.realtime_)this.realtime_.close();else if(this.establishConnectionTimer_&&(clearTimeout(this.establishConnectionTimer_),this.establishConnectionTimer_=null),this.connected_)this.onRealtimeDisconnect_()}resume(reason){log("Resuming connection for reason: "+
reason);delete this.interruptReasons_[reason];(0,_util.isEmpty)(this.interruptReasons_)&&(this.reconnectDelay_=1E3,this.realtime_||this.scheduleConnect_(0))}handleTimestamp_(timestamp){timestamp-=(new Date).getTime();this.onServerInfoUpdate_({serverTimeOffset:timestamp})}cancelSentTransactions_(){for(let i=0;i<this.outstandingPuts_.length;i++){const put=this.outstandingPuts_[i];if(put&&"h"in put.request&&put.queued){if(put.onComplete)put.onComplete("disconnect");delete this.outstandingPuts_[i];this.outstandingPutCount_--}}0===
this.outstandingPutCount_&&(this.outstandingPuts_=[])}onListenRevoked_(pathString,query){query=query?query.map(q=>ObjectToUniqueKey(q)).join("$"):"default";if((pathString=this.removeListen_(pathString,query))&&pathString.onComplete)pathString.onComplete("permission_denied")}removeListen_(pathString,queryId){pathString=(new Path(pathString)).toString();let listen;if(this.listens.has(pathString)){const map=this.listens.get(pathString);listen=map.get(queryId);map.delete(queryId);0===map.size&&this.listens.delete(pathString)}else listen=
void 0;return listen}onAuthRevoked_(statusCode,explanation){log("Auth token revoked: "+statusCode+"/"+explanation);this.authToken_=null;this.forceTokenRefresh_=!0;this.realtime_.close();if("invalid_token"===statusCode||"permission_denied"===statusCode)this.invalidAuthTokenCount_++,3<=this.invalidAuthTokenCount_&&(this.reconnectDelay_=3E4,this.authTokenProvider_.notifyForInvalidToken())}onAppCheckRevoked_(statusCode,explanation){log("App check token revoked: "+statusCode+"/"+explanation);this.appCheckToken_=
null;this.forceTokenRefresh_=!0;if("invalid_token"===statusCode||"permission_denied"===statusCode)this.invalidAppCheckTokenCount_++,3<=this.invalidAppCheckTokenCount_&&this.appCheckTokenProvider_.notifyForInvalidToken()}onSecurityDebugPacket_(body){this.securityDebugCallback_?this.securityDebugCallback_(body):"msg"in body&&console.log("FIREBASE: "+body.msg.replace("\n","\nFIREBASE: "))}restoreState_(){this.tryAuth();this.tryAppCheck();for(var queries of this.listens.values())for(const listenSpec of queries.values())this.sendListen_(listenSpec);
for(queries=0;queries<this.outstandingPuts_.length;queries++)this.outstandingPuts_[queries]&&this.sendPut_(queries);for(;this.onDisconnectRequestQueue_.length;)queries=this.onDisconnectRequestQueue_.shift(),this.sendOnDisconnect_(queries.action,queries.pathString,queries.data,queries.onComplete);for(queries=0;queries<this.outstandingGets_.length;queries++)this.outstandingGets_[queries]&&this.sendGet_(queries)}sendConnectStats_(){const stats={};let clientName="js";(0,_util.isNodeSdk)()&&(clientName=
this.repoInfo_.nodeAdmin?"admin_node":"node");stats["sdk."+clientName+"."+SDK_VERSION.replace(/\./g,"-")]=1;(0,_util.isMobileCordova)()?stats["framework.cordova"]=1:(0,_util.isReactNative)()&&(stats["framework.reactnative"]=1);this.reportStats(stats)}shouldReconnect_(){const online=OnlineMonitor.getInstance().currentlyOnline();return(0,_util.isEmpty)(this.interruptReasons_)&&online}}PersistentConnection.nextPersistentConnectionId_=0;PersistentConnection.nextConnectionId_=0;class NamedNode{constructor(name,
node){this.name=name;this.node=node}static Wrap(name,node){return new NamedNode(name,node)}}class Index{getCompare(){return this.compare.bind(this)}indexedValueChanged(oldNode,newNode){oldNode=new NamedNode("[MIN_NAME]",oldNode);newNode=new NamedNode("[MIN_NAME]",newNode);return 0!==this.compare(oldNode,newNode)}minPost(){return NamedNode.MIN}}let __EMPTY_NODE;class KeyIndex extends Index{static get __EMPTY_NODE(){return __EMPTY_NODE}static set __EMPTY_NODE(val){__EMPTY_NODE=val}compare(a,b){return nameCompare(a.name,
b.name)}isDefinedOn(node){throw(0,_util.assertionError)("KeyIndex.isDefinedOn not expected to be called.");}indexedValueChanged(oldNode,newNode){return!1}minPost(){return NamedNode.MIN}maxPost(){return new NamedNode("[MAX_NAME]",__EMPTY_NODE)}makePost(indexValue,name){(0,_util.assert)("string"===typeof indexValue,"KeyIndex indexValue must always be a string.");return new NamedNode(indexValue,__EMPTY_NODE)}toString(){return".key"}}const KEY_INDEX=new KeyIndex;class SortedMapIterator{constructor(node,
startKey,comparator,isReverse_,resultGenerator_=null){this.isReverse_=isReverse_;this.resultGenerator_=resultGenerator_;for(this.nodeStack_=[];!node.isEmpty();)if(resultGenerator_=startKey?comparator(node.key,startKey):1,isReverse_&&(resultGenerator_*=-1),0>resultGenerator_)node=this.isReverse_?node.left:node.right;else if(0===resultGenerator_){this.nodeStack_.push(node);break}else this.nodeStack_.push(node),node=this.isReverse_?node.right:node.left}getNext(){if(0===this.nodeStack_.length)return null;
let node=this.nodeStack_.pop(),result;result=this.resultGenerator_?this.resultGenerator_(node.key,node.value):{key:node.key,value:node.value};if(this.isReverse_)for(node=node.left;!node.isEmpty();)this.nodeStack_.push(node),node=node.right;else for(node=node.right;!node.isEmpty();)this.nodeStack_.push(node),node=node.left;return result}hasNext(){return 0<this.nodeStack_.length}peek(){if(0===this.nodeStack_.length)return null;const node=this.nodeStack_[this.nodeStack_.length-1];return this.resultGenerator_?
this.resultGenerator_(node.key,node.value):{key:node.key,value:node.value}}}class LLRBNode{constructor(key,value,color,left,right){this.key=key;this.value=value;this.color=null!=color?color:LLRBNode.RED;this.left=null!=left?left:SortedMap.EMPTY_NODE;this.right=null!=right?right:SortedMap.EMPTY_NODE}copy(key,value,color,left,right){return new LLRBNode(null!=key?key:this.key,null!=value?value:this.value,null!=color?color:this.color,null!=left?left:this.left,null!=right?right:this.right)}count(){return this.left.count()+
1+this.right.count()}isEmpty(){return!1}inorderTraversal(action){return this.left.inorderTraversal(action)||!!action(this.key,this.value)||this.right.inorderTraversal(action)}reverseTraversal(action){return this.right.reverseTraversal(action)||action(this.key,this.value)||this.left.reverseTraversal(action)}min_(){return this.left.isEmpty()?this:this.left.min_()}minKey(){return this.min_().key}maxKey(){return this.right.isEmpty()?this.key:this.right.maxKey()}insert(key,value,comparator){let n=this;
const cmp=comparator(key,n.key);n=0>cmp?n.copy(null,null,null,n.left.insert(key,value,comparator),null):0===cmp?n.copy(null,value,null,null,null):n.copy(null,null,null,null,n.right.insert(key,value,comparator));return n.fixUp_()}removeMin_(){if(this.left.isEmpty())return SortedMap.EMPTY_NODE;let n=this;n.left.isRed_()||n.left.left.isRed_()||(n=n.moveRedLeft_());n=n.copy(null,null,null,n.left.removeMin_(),null);return n.fixUp_()}remove(key,comparator){let n,smallest;n=this;if(0>comparator(key,n.key))n.left.isEmpty()||
n.left.isRed_()||n.left.left.isRed_()||(n=n.moveRedLeft_()),n=n.copy(null,null,null,n.left.remove(key,comparator),null);else{n.left.isRed_()&&(n=n.rotateRight_());n.right.isEmpty()||n.right.isRed_()||n.right.left.isRed_()||(n=n.moveRedRight_());if(0===comparator(key,n.key)){if(n.right.isEmpty())return SortedMap.EMPTY_NODE;smallest=n.right.min_();n=n.copy(smallest.key,smallest.value,null,null,n.right.removeMin_())}n=n.copy(null,null,null,null,n.right.remove(key,comparator))}return n.fixUp_()}isRed_(){return this.color}fixUp_(){let n=
this;n.right.isRed_()&&!n.left.isRed_()&&(n=n.rotateLeft_());n.left.isRed_()&&n.left.left.isRed_()&&(n=n.rotateRight_());n.left.isRed_()&&n.right.isRed_()&&(n=n.colorFlip_());return n}moveRedLeft_(){let n=this.colorFlip_();n.right.left.isRed_()&&(n=n.copy(null,null,null,null,n.right.rotateRight_()),n=n.rotateLeft_(),n=n.colorFlip_());return n}moveRedRight_(){let n=this.colorFlip_();n.left.left.isRed_()&&(n=n.rotateRight_(),n=n.colorFlip_());return n}rotateLeft_(){const nl=this.copy(null,null,LLRBNode.RED,
null,this.right.left);return this.right.copy(null,null,this.color,nl,null)}rotateRight_(){const nr=this.copy(null,null,LLRBNode.RED,this.left.right,null);return this.left.copy(null,null,this.color,null,nr)}colorFlip_(){const left=this.left.copy(null,null,!this.left.color,null,null),right=this.right.copy(null,null,!this.right.color,null,null);return this.copy(null,null,!this.color,left,right)}checkMaxDepth_(){const blackDepth=this.check_();return Math.pow(2,blackDepth)<=this.count()+1}check_(){if(this.isRed_()&&
this.left.isRed_())throw Error("Red node has red child("+this.key+","+this.value+")");if(this.right.isRed_())throw Error("Right child of ("+this.key+","+this.value+") is red");const blackDepth=this.left.check_();if(blackDepth!==this.right.check_())throw Error("Black depths differ");return blackDepth+(this.isRed_()?0:1)}}LLRBNode.RED=!0;LLRBNode.BLACK=!1;class LLRBEmptyNode{copy(key,value,color,left,right){return this}insert(key,value,comparator){return new LLRBNode(key,value,null)}remove(key,comparator){return this}count(){return 0}isEmpty(){return!0}inorderTraversal(action){return!1}reverseTraversal(action){return!1}minKey(){return null}maxKey(){return null}check_(){return 0}isRed_(){return!1}}
class SortedMap{constructor(comparator_,root_=SortedMap.EMPTY_NODE){this.comparator_=comparator_;this.root_=root_}insert(key,value){return new SortedMap(this.comparator_,this.root_.insert(key,value,this.comparator_).copy(null,null,LLRBNode.BLACK,null,null))}remove(key){return new SortedMap(this.comparator_,this.root_.remove(key,this.comparator_).copy(null,null,LLRBNode.BLACK,null,null))}get(key){let cmp,node=this.root_;for(;!node.isEmpty();){cmp=this.comparator_(key,node.key);if(0===cmp)return node.value;
0>cmp?node=node.left:0<cmp&&(node=node.right)}return null}getPredecessorKey(key){let cmp,node=this.root_,rightParent=null;for(;!node.isEmpty();){cmp=this.comparator_(key,node.key);if(0===cmp){if(node.left.isEmpty())return rightParent?rightParent.key:null;for(node=node.left;!node.right.isEmpty();)node=node.right;return node.key}0>cmp?node=node.left:0<cmp&&(rightParent=node,node=node.right)}throw Error("Attempted to find predecessor key for a nonexistent key.  What gives?");}isEmpty(){return this.root_.isEmpty()}count(){return this.root_.count()}minKey(){return this.root_.minKey()}maxKey(){return this.root_.maxKey()}inorderTraversal(action){return this.root_.inorderTraversal(action)}reverseTraversal(action){return this.root_.reverseTraversal(action)}getIterator(resultGenerator){return new SortedMapIterator(this.root_,
...
"https://":"http://")+this.repoInfo_.host}}const parseRepoInfo=function(dataURL,nodeAdmin){var JSCompiler_object_inline_host_2159,JSCompiler_object_inline_subdomain_2162,JSCompiler_object_inline_pathString_2165,dataURL$jscomp$0=dataURL,domain=JSCompiler_object_inline_host_2159="";dataURL=JSCompiler_object_inline_pathString_2165=JSCompiler_object_inline_subdomain_2162="";var JSCompiler_object_inline_secure_2163=!0;var JSCompiler_object_inline_scheme_2164="https";if("string"===typeof dataURL$jscomp$0){domain=
dataURL$jscomp$0.indexOf("//");0<=domain&&(JSCompiler_object_inline_scheme_2164=dataURL$jscomp$0.substring(0,domain-1),dataURL$jscomp$0=dataURL$jscomp$0.substring(domain+2));var slashInd=dataURL$jscomp$0.indexOf("/");-1===slashInd&&(slashInd=dataURL$jscomp$0.length);domain=dataURL$jscomp$0.indexOf("?");-1===domain&&(domain=dataURL$jscomp$0.length);JSCompiler_object_inline_host_2159=dataURL$jscomp$0.substring(0,Math.min(slashInd,domain));if(slashInd<domain){JSCompiler_object_inline_pathString_2165=
"";slashInd=dataURL$jscomp$0.substring(slashInd,domain).split("/");for(let i=0;i<slashInd.length;i++)if(0<slashInd[i].length){let piece=slashInd[i];try{piece=decodeURIComponent(piece.replace(/\+/g," "))}catch(e){}JSCompiler_object_inline_pathString_2165+="/"+piece}}dataURL$jscomp$0=dataURL$jscomp$0.substring(Math.min(dataURL$jscomp$0.length,domain));domain={};"?"===dataURL$jscomp$0.charAt(0)&&(dataURL$jscomp$0=dataURL$jscomp$0.substring(1));for(JSCompiler_object_inline_domain_2161 of dataURL$jscomp$0.split("\x26"))0!==
JSCompiler_object_inline_domain_2161.length&&(slashInd=JSCompiler_object_inline_domain_2161.split("\x3d"),2===slashInd.length?domain[decodeURIComponent(slashInd[0])]=decodeURIComponent(slashInd[1]):warn(`Invalid query segment '${JSCompiler_object_inline_domain_2161}' in query '${dataURL$jscomp$0}'`));var JSCompiler_object_inline_domain_2161=domain;domain=JSCompiler_object_inline_host_2159.indexOf(":");0<=domain?(JSCompiler_object_inline_secure_2163="https"===JSCompiler_object_inline_scheme_2164||
"wss"===JSCompiler_object_inline_scheme_2164,parseInt(JSCompiler_object_inline_host_2159.substring(domain+1),10)):domain=JSCompiler_object_inline_host_2159.length;dataURL$jscomp$0=JSCompiler_object_inline_host_2159.slice(0,domain);"localhost"===dataURL$jscomp$0.toLowerCase()?domain="localhost":2>=dataURL$jscomp$0.split(".").length?domain=dataURL$jscomp$0:(dataURL=JSCompiler_object_inline_host_2159.indexOf("."),JSCompiler_object_inline_subdomain_2162=JSCompiler_object_inline_host_2159.substring(0,
dataURL).toLowerCase(),domain=JSCompiler_object_inline_host_2159.substring(dataURL+1),dataURL=JSCompiler_object_inline_subdomain_2162);"ns"in JSCompiler_object_inline_domain_2161&&(dataURL=JSCompiler_object_inline_domain_2161.ns)}JSCompiler_object_inline_domain_2161=domain;"firebase.com"===JSCompiler_object_inline_domain_2161&&fatal(JSCompiler_object_inline_host_2159+" is no longer supported. Please use \x3cYOUR FIREBASE\x3e.firebaseio.com instead");dataURL&&"undefined"!==dataURL||"localhost"===JSCompiler_object_inline_domain_2161||
fatal("Cannot parse Firebase url. Please use https://\x3cYOUR FIREBASE\x3e.firebaseio.com");JSCompiler_object_inline_secure_2163||"undefined"!==typeof window&&window.location&&window.location.protocol&&-1!==window.location.protocol.indexOf("https:")&&warn("Insecure Firebase access from a secure page. Please use https in calls to new Firebase().");return{repoInfo:new RepoInfo(JSCompiler_object_inline_host_2159,JSCompiler_object_inline_secure_2163,dataURL,nodeAdmin,"ws"===JSCompiler_object_inline_scheme_2164||
"wss"===JSCompiler_object_inline_scheme_2164,"",dataURL!==JSCompiler_object_inline_subdomain_2162),path:new Path(JSCompiler_object_inline_pathString_2165)}};class DataEvent{constructor(eventType,eventRegistration,snapshot,prevName){this.eventType=eventType;this.eventRegistration=eventRegistration;this.snapshot=snapshot;this.prevName=prevName}getPath(){const ref=this.snapshot.ref;return"value"===this.eventType?ref._path:ref.parent._path}getEventType(){return this.eventType}getEventRunner(){return this.eventRegistration.getEventRunner(this)}toString(){return this.getPath().toString()+
...
(repoStart(this._repoInternal,this.app.options.appId,this.app.options.databaseAuthVariableOverride),this._instanceStarted=!0);return this._repoInternal}get _root(){this._rootInternal||(this._rootInternal=new ReferenceImpl(this._repo,newEmptyPath()));return this._rootInternal}_delete(){if(null!==this._rootInternal){var repo=this._repo,appName=this.app.name;const appRepos=repos[appName];appRepos&&appRepos[repo.key]===repo||fatal(`Database ${appName}(${repo.repoInfo_}) has already been deleted.`);repo.persistentConnection_&&
repo.persistentConnection_.interrupt("repo_interrupt");delete appRepos[repo.key];this._rootInternal=this._repoInternal=null}return Promise.resolve()}_checkNotDeleted(apiName){null===this._rootInternal&&fatal("Cannot call "+apiName+" on a deleted database.")}}exports.Database=Database;const SERVER_TIMESTAMP={".sv":"timestamp"};class TransactionResult{constructor(committed,snapshot){this.committed=committed;this.snapshot=snapshot}toJSON(){return{committed:this.committed,snapshot:this.snapshot.toJSON()}}}
exports.TransactionResult=TransactionResult;PersistentConnection;PersistentConnection.prototype.simpleListen=function(pathString,onComplete){this.sendRequest("q",{p:pathString},onComplete)};PersistentConnection.prototype.echo=function(data,onEcho){this.sendRequest("echo",{d:data},onEcho)};Connection;exports._TEST_ACCESS_hijackHash=function(newHash){const oldPut=PersistentConnection.prototype.put;PersistentConnection.prototype.put=function(pathString,data,onComplete,hash){void 0!==hash&&(hash=newHash());
oldPut.call(this,pathString,data,onComplete,hash)};return function(){PersistentConnection.prototype.put=oldPut}};RepoInfo;exports._TEST_ACCESS_forceRestClient=function(forceRestClient){useRestClient=forceRestClient};(function(variant){SDK_VERSION=_app.SDK_VERSION;(0,_app._registerComponent)((new _component.Component("database",(container,{instanceIdentifier:url})=>{const app=container.getProvider("app").getImmediate(),authProvider=container.getProvider("auth-internal");container=container.getProvider("app-check-internal");
return repoManagerDatabaseFromApp(app,authProvider,container,url)},"PUBLIC")).setMultipleInstances(!0));(0,_app.registerVersion)("@firebase/database","0.12.5",variant);(0,_app.registerVersion)("@firebase/database","0.12.5","esm2017")})()}
