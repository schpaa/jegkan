...
selectedPersistence)try{yield persistence._remove(key)}catch(_a){}})));return new PersistenceUserManager(selectedPersistence,auth,userKey)})}}class AuthImpl{constructor(app,config){this.app=app;this.config=config;this.emulatorConfig=this.currentUser=null;this.operations=Promise.resolve();this.authStateSubscription=new Subscription(this);this.idTokenSubscription=new Subscription(this);this.redirectUser=null;this.isProactiveRefreshEnabled=!1;this._canInitEmulator=!0;this._deleted=this._isInitialized=
!1;this._popupRedirectResolver=this._initializationPromise=null;this._errorFactory=_DEFAULT_AUTH_ERROR_FACTORY;this.lastNotifiedUid=void 0;this.tenantId=this.languageCode=null;this.settings={appVerificationDisabledForTesting:!1};this.frameworks=[];this.name=app.name;this.clientVersion=config.sdkClientVersion}_initializeWithPersistence(persistenceHierarchy,popupRedirectResolver){popupRedirectResolver&&(this._popupRedirectResolver=_getInstance(popupRedirectResolver));return this._initializationPromise=
this.queue(()=>{const $jscomp$async$this=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){var _a,_b;if(!$jscomp$async$this._deleted&&($jscomp$async$this.persistenceManager=yield PersistenceUserManager.create($jscomp$async$this,persistenceHierarchy),!$jscomp$async$this._deleted)){if(null===(_a=$jscomp$async$this._popupRedirectResolver)||void 0===_a?0:_a._shouldInitProactively)try{yield $jscomp$async$this._popupRedirectResolver._initialize($jscomp$async$this)}catch(e){}yield $jscomp$async$this.initializeCurrentUser(popupRedirectResolver);
...
this.clientVersion=_getClientVersion(this.config.clientPlatform,this._getFrameworks()))}_getFrameworks(){return this.frameworks}_getAdditionalHeaders(){const $jscomp$async$this=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const headers={["X-Client-Version"]:$jscomp$async$this.clientVersion};$jscomp$async$this.app.options.appId&&(headers["X-Firebase-gmpid"]=$jscomp$async$this.app.options.appId);return headers})}}exports.aE=AuthImpl;class Subscription{constructor(auth){this.auth=
auth;this.observer=null;this.addObserver=(0,_util.createSubscribe)(observer=>this.observer=observer)}get next(){_assert(this.observer,this.auth,"internal-error");return this.observer.next.bind(this.observer)}}class AuthCredential{constructor(providerId,signInMethod){this.providerId=providerId;this.signInMethod=signInMethod}toJSON(){return debugFail("not implemented")}_getIdTokenResponse(_auth){return debugFail("not implemented")}_linkToIdToken(_auth,_idToken){return debugFail("not implemented")}_getReauthenticationResolver(_auth){return debugFail("not implemented")}}
exports.G=AuthCredential;class EmailAuthCredential extends AuthCredential{constructor(_email,_password,signInMethod,_tenantId=null){super("password",signInMethod);this._email=_email;this._password=_password;this._tenantId=_tenantId}static _fromEmailAndPassword(email,password){return new EmailAuthCredential(email,password,"password")}static _fromEmailAndCode(email,oobCode,tenantId=null){return new EmailAuthCredential(email,oobCode,"emailLink",tenantId)}toJSON(){return{email:this._email,password:this._password,
signInMethod:this.signInMethod,tenantId:this._tenantId}}static fromJSON(json){json="string"===typeof json?JSON.parse(json):json;if((null===json||void 0===json?0:json.email)&&(null===json||void 0===json?0:json.password)){if("password"===json.signInMethod)return this._fromEmailAndPassword(json.email,json.password);if("emailLink"===json.signInMethod)return this._fromEmailAndCode(json.email,json.password,json.tenantId)}return null}_getIdTokenResponse(auth){const $jscomp$async$this=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){switch($jscomp$async$this.signInMethod){case "password":return signInWithPassword(auth,
{returnSecureToken:!0,email:$jscomp$async$this._email,password:$jscomp$async$this._password});case "emailLink":return signInWithEmailLink$1(auth,{email:$jscomp$async$this._email,oobCode:$jscomp$async$this._password});default:_fail(auth,"internal-error")}})}_linkToIdToken(auth,idToken){const $jscomp$async$this=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){switch($jscomp$async$this.signInMethod){case "password":return updateEmailPassword(auth,{idToken,returnSecureToken:!0,email:$jscomp$async$this._email,
password:$jscomp$async$this._password});case "emailLink":return signInWithEmailLinkForLinking(auth,{idToken,email:$jscomp$async$this._email,oobCode:$jscomp$async$this._password});default:_fail(auth,"internal-error")}})}_getReauthenticationResolver(auth){return this._getIdTokenResponse(auth)}}exports.H=EmailAuthCredential;class OAuthCredential extends AuthCredential{constructor(){super(...arguments);this.pendingToken=null}static _fromParams(params){const cred=new OAuthCredential(params.providerId,
...
secret:this.secret,nonce:this.nonce,pendingToken:this.pendingToken,providerId:this.providerId,signInMethod:this.signInMethod}}static fromJSON(json){json="string"===typeof json?JSON.parse(json):json;const {providerId,signInMethod}=json;json=(0,_tslib.__rest)(json,["providerId","signInMethod"]);if(!providerId||!signInMethod)return null;const cred=new OAuthCredential(providerId,signInMethod);cred.idToken=json.idToken||void 0;cred.accessToken=json.accessToken||void 0;cred.secret=json.secret;cred.nonce=
json.nonce;cred.pendingToken=json.pendingToken||null;return cred}_getIdTokenResponse(auth){const request=this.buildRequest();return signInWithIdp(auth,request)}_linkToIdToken(auth,idToken){const request=this.buildRequest();request.idToken=idToken;return signInWithIdp(auth,request)}_getReauthenticationResolver(auth){const request=this.buildRequest();request.autoCreate=!1;return signInWithIdp(auth,request)}buildRequest(){const request={requestUri:"http://localhost",returnSecureToken:!0};if(this.pendingToken)request.pendingToken=
...
"phone");this.params=params}static _fromVerification(verificationId,verificationCode){return new PhoneAuthCredential({verificationId,verificationCode})}static _fromTokenResponse(phoneNumber,temporaryProof){return new PhoneAuthCredential({phoneNumber,temporaryProof})}_getIdTokenResponse(auth){return signInWithPhoneNumber$1(auth,this._makeVerificationRequest())}_linkToIdToken(auth,idToken){return linkWithPhoneNumber$1(auth,Object.assign({idToken},this._makeVerificationRequest()))}_getReauthenticationResolver(auth){return verifyPhoneNumberForExisting(auth,
this._makeVerificationRequest())}_makeVerificationRequest(){const {temporaryProof,phoneNumber,verificationId,verificationCode}=this.params;return temporaryProof&&phoneNumber?{temporaryProof,phoneNumber}:{sessionInfo:verificationId,code:verificationCode}}toJSON(){const obj={providerId:this.providerId};this.params.phoneNumber&&(obj.phoneNumber=this.params.phoneNumber);this.params.temporaryProof&&(obj.temporaryProof=this.params.temporaryProof);this.params.verificationCode&&(obj.verificationCode=this.params.verificationCode);
this.params.verificationId&&(obj.verificationId=this.params.verificationId);return obj}static fromJSON(json){"string"===typeof json&&(json=JSON.parse(json));const {verificationId,verificationCode,phoneNumber,temporaryProof}=json;return verificationCode||verificationId||phoneNumber||temporaryProof?new PhoneAuthCredential({verificationId,verificationCode,phoneNumber,temporaryProof}):null}}exports.J=PhoneAuthCredential;class ActionCodeURL{constructor(actionLink){var _a,_b,_c,_d,_e,_f;actionLink=(0,_util.querystringDecode)((0,_util.extractQuerystring)(actionLink));
const apiKey=null!==(_a=actionLink.apiKey)&&void 0!==_a?_a:null;_a=null!==(_b=actionLink.oobCode)&&void 0!==_b?_b:null;_b=parseMode(null!==(_c=actionLink.mode)&&void 0!==_c?_c:null);_assert(apiKey&&_a&&_b,"argument-error");this.apiKey=apiKey;this.operation=_b;this.code=_a;this.continueUrl=null!==(_d=actionLink.continueUrl)&&void 0!==_d?_d:null;this.languageCode=null!==(_e=actionLink.languageCode)&&void 0!==_e?_e:null;this.tenantId=null!==(_f=actionLink.tenantId)&&void 0!==_f?_f:null}static parseLink(link$jscomp$0){const link=
(0,_util.querystringDecode)((0,_util.extractQuerystring)(link$jscomp$0)).link,doubleDeepLink=link?(0,_util.querystringDecode)((0,_util.extractQuerystring)(link)).deep_link_id:null,iOSDeepLink=(0,_util.querystringDecode)((0,_util.extractQuerystring)(link$jscomp$0)).deep_link_id;link$jscomp$0=(iOSDeepLink?(0,_util.querystringDecode)((0,_util.extractQuerystring)(iOSDeepLink)).link:null)||iOSDeepLink||doubleDeepLink||link||link$jscomp$0;try{return new ActionCodeURL(link$jscomp$0)}catch(_a){return null}}}
exports.ac=ActionCodeURL;class EmailAuthProvider{constructor(){this.providerId=EmailAuthProvider.PROVIDER_ID}static credential(email,password){return EmailAuthCredential._fromEmailAndPassword(email,password)}static credentialWithLink(email,emailLink){emailLink=ActionCodeURL.parseLink(emailLink);_assert(emailLink,"argument-error");return EmailAuthCredential._fromEmailAndCode(email,emailLink.code,emailLink.tenantId)}}exports.L=EmailAuthProvider;EmailAuthProvider.PROVIDER_ID="password";EmailAuthProvider.EMAIL_PASSWORD_SIGN_IN_METHOD=
...
unsubscribe&&(this.internalListeners.delete(listener),unsubscribe(),this.updateProactiveRefresh())}assertAuthConfigured(){_assert(this.auth._initializationPromise,"dependent-sdk-initialized-before-auth")}updateProactiveRefresh(){0<this.internalListeners.size?this.auth._startProactiveRefresh():this.auth._stopProactiveRefresh()}}(function(clientPlatform){(0,_app._registerComponent)((new _component.Component("auth",(container,{options:deps})=>{container=container.getProvider("app").getImmediate();const {apiKey,
authDomain}=container.options;_assert(apiKey&&!apiKey.includes(":"),"invalid-api-key",{appName:container.name});_assert(!(null===authDomain||void 0===authDomain?0:authDomain.includes(":")),"argument-error",{appName:container.name});var config={apiKey,authDomain,clientPlatform,apiHost:"identitytoolkit.googleapis.com",tokenApiHost:"securetoken.googleapis.com",apiScheme:"https",sdkClientVersion:_getClientVersion(clientPlatform)};container=new AuthImpl(container,config);config=(null===deps||void 0===
deps?void 0:deps.persistence)||[];config=(Array.isArray(config)?config:[config]).map(_getInstance);(null===deps||void 0===deps?0:deps.errorMap)&&container._updateErrorMap(deps.errorMap);container._initializeWithPersistence(config,null===deps||void 0===deps?void 0:deps.popupRedirectResolver);return container},"PUBLIC")).setInstantiationMode("EXPLICIT").setInstanceCreatedCallback((container,_instanceIdentifier,_instance)=>{container.getProvider("auth-internal").initialize()}));(0,_app._registerComponent)((new _component.Component("auth-internal",
container=>{container=_castAuth(container.getProvider("auth").getImmediate());return new AuthInterop(container)},"PRIVATE")).setInstantiationMode("EXPLICIT"));(0,_app.registerVersion)("@firebase/auth","0.19.8",getVersionForPlatform(clientPlatform));(0,_app.registerVersion)("@firebase/auth","0.19.8","esm2017")})("Browser")}
//# sourceMappingURL=module$node_modules$$firebase$auth$dist$esm2017$index_342f2197.js.map
